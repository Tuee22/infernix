...
| Tests | present | lint, unit, exhaustive integration, and routed E2E entrypoints exist, and they now validate process-isolated engine-worker adapters, repo-owned engine fixture command injection, durable runtime bundles, direct-upstream source-artifact manifests, and host-gated `linux-cuda` scheduling or device visibility checks, but final closure remains open because they still do not exercise final engine execution across the full supported matrix on supported hosts |
...
- `cluster up` fails fast in `linux-cuda` unless the host passes the NVIDIA preflight contract for
  `nvidia-smi`, Docker `--runtime=nvidia`, and the CDI device mount used by the Kind worker.
