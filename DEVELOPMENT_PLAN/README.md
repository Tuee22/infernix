...
Silicon hardware and the prior Linux/CUDA host are both no longer available. The retired dated
proof points (May 13/15/17/19/25/26/27/29, 2026) are inventoried in
...
| 7 | Demo App Multi-User Durable Context | Active (Sprints 7.1, 7.3–7.13, 7.16–7.17 code-side closed including the full daemon-split + durable-context shell + routed Keycloak/WebSocket/MinIO browser flows + Sprint 7.17 configuration-doctrine retirement; Apple cohort `validateDurableContextPromptRoundTrip` PASS + 5/6 e2e PASS closed in [Wave A](cohort-validation-waves.md); [Wave B](cohort-validation-waves.md) residuals active — Sprint 7.15 artifact-upload e2e fix, Sprint 7.15 per-model smoke matrix, Sprint 7.14 chaos + throughput suites; CUDA Linux cohort pending [Wave C](cohort-validation-waves.md)) | [phase-7-demo-app-durable-context.md](phase-7-demo-app-durable-context.md) |
