...
on the new host before any current Apple proof points can be claimed. The Apple cohort `cabal test infernix-integration` full-suite PASS landed 2026-05-30 on the new
Apple Silicon host (after the `repoEngineReplicaCount FinalPhase + AppleSilicon -> 0` follow-on
landed on the same day), covering routed integration assertions including the per-model inference
matrix, the durable-context prompt roundtrip with `inferenceResultStatus = completed`, the host
engine daemon trace, edge-port conflict rediscovery, retained-state replay across cluster
lifecycle, and Harbor-publication arm64 closure of all 9 platform images. Routed Playwright E2E
on Apple host-native, `infernix test all` end-to-end including the chart-rendered Web bundle,
the multi-replica HA Coordinator validation, and CUDA Linux cohort full-suite validation remain
the open Apple Silicon and CUDA Linux residuals on the new host. The
...
| 2 | Kind Cluster Storage and Lifecycle | Active (Sprint 2.13 code-side env capture retirement + HostTool routing landed; Sprint 2.13 follow-on landed 2026-05-29 — subprocess PATH derives from `HostConfig.toolPaths.*`, Apple `hostDocker` corrected to `/opt/homebrew/bin/docker`, operator home from libc, `curl -m 30` Harbor probe, `LineBuffering` stdout/stderr — cross-listed with Phase 3 Sprint 3.11; the prior clean-env `linux-gpu` lifecycle validation (May 27, 2026) was on the retired hardware and no longer counts as a current proof point; Apple cohort and CUDA Linux cohort validation both pending on new Apple Silicon host) | [phase-2-kind-cluster-storage-and-lifecycle.md](phase-2-kind-cluster-storage-and-lifecycle.md) |
| 3 | HA Platform Services and Edge Routing | Active (Sprint 3.10 code landed; Sprint 3.11 landed 2026-05-29 — substrate-aware publication arch via `clusterWorkloadArchitecture` (Apple Silicon → arm64, Linux → amd64), `bitnamilegacy/*` retirement + hand-authored MinIO StatefulSet under `chart/templates/minio/` replacing the retired bitnami sub-chart, Harbor host-port dynamic discovery via `chooseHarborPort` + `./.data/runtime/harbor-port.json` mirroring Section O, containerd `config_path = "/etc/containerd/certs.d"` patch in rendered Kind config; **Apple cohort `cluster up → status (steady-state) → cluster down → status (cluster-absent)` lifecycle proof point validated 2026-05-29** on the new Apple Silicon host with arm64-native publication of all 9 platform images and `chooseHarborPort` selecting `30003`; the full `infernix test all` integration replay had previously exposed an independent Phase 2 Sprint 2.13 follow-on for retained-state Keycloak Patroni replay; the fix landed 2026-05-30 (`isPatroniManagedClaim` + `scrubStalePatroniDirectories` in `src/Infernix/Cluster.hs`); the prior CUDA Linux cohort in-container Playwright E2E (May 27, 2026) was on the retired hardware and no longer counts as a current proof point; **Apple cohort `cabal test infernix-integration` full-suite PASS validated 2026-05-30 on the new Apple Silicon host** after the `repoEngineReplicaCount FinalPhase + AppleSilicon -> 0` follow-on landed (eliminating the in-cluster engine pod that competed with the host daemon on the host-batch Shared subscription); CUDA Linux cohort validation pending on new host through Colima's amd64 VM) | [phase-3-ha-platform-services-and-edge-routing.md](phase-3-ha-platform-services-and-edge-routing.md) |
