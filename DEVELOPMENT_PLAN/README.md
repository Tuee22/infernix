...
| 9 | Access Control and Monitoring | Active — code-side closed 2026-07-06 (all 8 sprints; machine-independent gates green: `cabal build all`, `cabal test infernix-unit`, `cabal test infernix-haskell-style`, `infernix lint chart\|docs\|files\|proto`, `infernix docs check`, `poetry run check-code`). Admin/user RBAC (Keycloak `infernix-admin` realm role + JWT `realm_access.roles` claim + hardcoded demo admin), edge admin `SecurityPolicy` over all four operator routes + ungated-route closure, backend admin gate on `GET /api/cache` + `/api/cache/{evict,rebuild}` + the new `GET /api/admin/overview`, admin cluster-wide monitoring panel + per-user personal dashboard (verbatim `index.html`), the Kind data-plane + edge loopback invariant enforced by `lint chart` + a unit assertion, and per-user MinIO STS defense-in-depth (`cluster.minio.stsPerUser`, now default on). **Wave Q apple-silicon live-validated 2026-07-07**: a full `cluster up` on this Apple host proved unauthenticated 401 + by-role 403/2xx over all operator routes + `/api/cache` + `/api/admin/overview`, the admin `realm_access.roles` claim, real admin-overview aggregates, the loopback data plane un-gated while the edge is admin-gated, per-user object isolation, the STS scoped-credential object path, and routed Playwright RBAC/dashboard/lifecycle **7/7**. The `linux-cpu` shared-lane rerun (substrate-independent behavior) is the remaining residual for full `Done`. | [phase-9-access-control-and-monitoring.md](phase-9-access-control-and-monitoring.md) |
