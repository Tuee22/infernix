...
| 4 | Inference Service and Durable Runtime | Active — Sprint 4.22 MT3 catalog replacement: **both `music-mt3-infer` and `music-mr-mt3` proven 2026-07-02** (`linux-cpu` full-suite GREEN incl. routed Playwright `9/9`; `linux-gpu` integration real MIDI on CUDA + `8/9` e2e). The only remaining item is the clean `linux-gpu` `9/9`, blocked solely by the MT3-unrelated CUDA-only `video-wan21-t2v` cold-cache timeout — owned by Phase 8 Sprint 8.5 (eager model-cache staging), whose unblocking code (coordinator eager sweep + `warm-model-cache` `cluster up` barrier) landed and is code-side closed 2026-07-03; the clean `linux-gpu` `9/9` is now the shared cohort residual. Sprints 4.1-4.21/4.23/4.24 remain closed for their original scopes. | [phase-4-inference-service-and-durable-runtime.md](phase-4-inference-service-and-durable-runtime.md) |
...
| 6 | Validation, E2E, and HA Hardening | Active — Sprint 6.35 expanded MT3 catalog integration/e2e gate: **both MT3 rows proven 2026-07-02** (`linux-cpu` full-suite GREEN incl. routed Playwright `9/9`; `linux-gpu` integration + their routed matrix rows real on CUDA). The clean `linux-gpu` `9/9` is gated only on Phase 8 Sprint 8.5 eager model-cache staging (the CUDA-only `video-wan21-t2v` cold-cache row), not on any MT3 code; that eager-staging code landed and is code-side closed 2026-07-03, leaving the clean `linux-gpu` `9/9` as the shared cohort residual. Sprints 6.1-6.34 remain closed for their original scopes. | [phase-6-validation-e2e-and-ha-hardening.md](phase-6-validation-e2e-and-ha-hardening.md) |
...
| 8 | Zero-Tracked-Dhall Config and Eager Model Cache | Active — code-side closed 2026-07-03; single-accelerator cohort gate pending. Sprints 8.1-8.4 and 8.6 Done (zero-tracked `.dhall`; `infernix init` / `test init` explicit creation with shared defaults; fail-fast no-auto-generate backstops; binary-generated ConfigMap/Secret bodies with the chart as string embedder; test-harness config lifecycle). Sprint 8.5 (coordinator eager model-cache staging + `warm-model-cache` `cluster up` barrier + `--empty-models` image bake) is code-side closed and Active on the shared `linux-gpu` plus `linux-cpu` warm-cache + 9/9 cohort residual (jointly with Phase 4 Sprint 4.22 / Phase 6 Sprint 6.35). Machine-independent gates pass. | [phase-8-zero-tracked-dhall-config-and-eager-model-cache.md](phase-8-zero-tracked-dhall-config-and-eager-model-cache.md) |
