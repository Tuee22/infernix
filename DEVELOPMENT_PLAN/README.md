...
cohort wave (Metal, including the tart Metal-engine build and the `apple-silicon` catalog rows).
...
| 1 | Repository and Control-Plane Foundation | Active (Sprint 1.13 Apple tart Metal-engine build lane code-side closure is **complete** — the `tart` host-manifest field, `AppleTart` reconciliation, `infernix internal materialize-metal-engines` registration, the allowlisted tart build lane, and unit coverage, proven by the machine-independent gate set on the present CUDA Linux host; the in-VM tart Metal build is the **Apple-only** Stage 2 cohort gate in [Wave I](cohort-validation-waves.md) — no CUDA residual; the prior Sprint 1.12 native-only workflow doctrine closed on the recorded validation: Apple `doctor`/`build`/`up`/`status`/full `test`/`down`/final `status` passed on an already selected native arm64 daemon, and the negative no-daemon bootstrap boundary failed before cluster work without changing Docker contexts or Colima VM state) | [phase-1-repository-and-control-plane-foundation.md](phase-1-repository-and-control-plane-foundation.md) |
...
| 4 | Inference Service and Durable Runtime | Active (real per-family inference code-side closure **complete** across Sprints 4.1/4.2/4.3/4.7/4.8/4.10/4.11/4.12/4.14/4.15 plus Sprint 4.16 (per-engine isolated framework venvs); Sprint 4.17 code-side implementation now includes the per-engine image split, `engineDaemons` substrate schema, coordinator→`inference.batch.<mode>.<engine>` routing, per-engine service selection, chart per-engine Deployments/PDBs, lifecycle image builds, Harbor per-engine image overlays, single-GPU validation serialization that scales one per-engine deployment at a time, worker protobuf model-cache/MinIO wiring for Python adapters, and image-owned native-runner root fallback under `/opt/infernix/engines/<adapterId>/`. June 11, 2026 local validation has temp-copy Linux GPU launcher `cabal build all`, `cabal test infernix-unit`, and `cabal test infernix-haskell-style` passing; current-source `cabal run exe:infernix -- lint docs`, `docs check`, `lint chart`, `lint files`, `lint proto`, and linux-gpu substrate materialize/validate checks exit 0. Current-source mounted linux-gpu validation after the latest full run passes `cabal test infernix-unit`, `cabal test infernix-haskell-style`, `cabal build test:infernix-integration`, `poetry --directory python run check-code`, `cabal run exe:infernix -- lint docs`, `docs check`, `lint files`, `lint proto`, and `lint chart`. The Harbor Patroni scrub gap, single-GPU concurrent per-engine scheduling gap, Harbor registry bucket/multipart/tmp scrub gap, BuildKit provenance descriptor gap, and retained Harbor Redis blob-cache gap are fixed in code. The latest governed `./bootstrap/linux-gpu.sh test` rerun passed Haskell style, Haskell unit, and web unit gates; push/pull-verified all five per-engine images, the slim control-plane image, and chart upstream images through Harbor; completed final rollout and route probes; then failed at `llm-tinyllama-gguf` because the Linux base engine pod had no `/workspace/.data/engines/llama-cpp-cli/bin/llama-cli`. The post-harness-fix governed launcher rebuild passed as plain manifest `sha256:2d6cfd42ca59ee7fbd9669a8c32738ed0ba44ef09706b469d12c8803b520e030`. The Stage 2 cohort batch in [Wave I](cohort-validation-waves.md) now owns the Linux native-engine binary/materialization lane plus real-engine output for every row, full routed linux-gpu per-engine cluster evidence, Apple tart Metal artifacts, and the Basic Pitch TensorFlow/Omnizart/MT3 named residuals; prior Sprints 4.1-4.14 closed with mounted `ClusterConfig` / `SecretsConfig` runtime path validated by Apple Wave A and CUDA Linux Wave C) | [phase-4-inference-service-and-durable-runtime.md](phase-4-inference-service-and-durable-runtime.md) |
...
| 6 | Validation, E2E, and HA Hardening | Active (per-family real-output coverage code-side closure **complete** across Sprints 6.2/6.3/6.6 — the `ResultFamily`-dispatched `validateCatalogModelInference`, the substrate-agnostic per-family Playwright + Chat artifact rendering, and the `allMatrixRowIds` union invariant + README-to-matrix `lint docs` check — proven on the present CUDA Linux host (Stage 1: `cabal test infernix-unit`, `cabal build test:infernix-integration`, `infernix lint docs/files`; 6.3's spago web-unit suite runs in the container/Node-22 lane on this host). The June 11, 2026 linux-gpu live run reached the per-family assertion phase after Harbor publication and `cluster up`; follow-up integration code now uploads canonical input objects for audio/image input rows and asserts `status = completed` before result-family shape, with current-source mounted linux-gpu `cabal test infernix-unit`, `cabal test infernix-haskell-style`, `cabal build test:infernix-integration`, `poetry --directory python run check-code`, `cabal run exe:infernix -- lint docs`, `docs check`, `lint files`, `lint proto`, and `lint chart` passing, then governed launcher `sha256:2d6cfd42ca59ee7fbd9669a8c32738ed0ba44ef09706b469d12c8803b520e030` building as a plain manifest. The latest full governed rerun proves style/unit/web plus Harbor/final-rollout/route-probe gates and now fails first on missing Linux native engine binaries at `llm-tinyllama-gguf`. The real-engine integration/E2E assertions remain the Stage 2 cohort batch in [Wave I](cohort-validation-waves.md); the prior Sprints 6.1-6.30 closed with lint/style/unit/integration/e2e, no-env-var gates, and single `ghc-9.12.4` toolchain validation) | [phase-6-validation-e2e-and-ha-hardening.md](phase-6-validation-e2e-and-ha-hardening.md) |
