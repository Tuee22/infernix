...
barrier are the landed slice of **Phase 6 Sprint 6.41** (`Active (partial)`, code-side closed
2026-07-19, machine-independent and adversarially reviewed): `src/Infernix/ProcessMonitor.hs` is
deleted (its ten monitored lifecycle commands rerouted through the bounded
`runCommandBounded`/`tryCommandBounded` helpers and its `unboundedExecExemptedFiles` row dropped),
`retryCommandOutput` is migrated onto `awaitReadiness` (bounding all six of its consumers), and
`waitForEagerModelCacheReady` is migrated onto the same kernel. The outstanding tail is the remaining
individual hand-rolled bounded waits (already finite, so not flake-risk) and the
`threadDelay`-outside-kernel lint gate; the cohort gate is pending. The retired module is now recorded
in the deletion ledger's `## Completed` under Sprint 6.41.
...
The [bounded inference memory doctrine](../documents/architecture/bounded_inference_memory.md) closes
it by construction: an inference engine subprocess runs only under a typed `MemoryGrant` minted by
`admitModelMemory`, and the capped-engine kernel bounds its actual resident memory to the admitted
`MemoryCeiling`; an over-budget model is a clean `status=failed` `ModelMemoryLimitExceeded`, never a
host OOM. The memory model is made total and honest — a required `ModelMemoryFootprint` newtype (no
bare-`Int` default-0), a budget that names its enforcer
(`HostEnforcedBudget HostMemoryPartition | SubstrateEnforcedBudget PodMemoryLimit`, dropping
`UnenforcedMemoryBudget`), and a checked `HostMemoryPartition` (physical = vmReserve + hostHeadroom +
inferenceCapacity, rejecting oversubscription; headroom covering OS + routed-E2E browser). macOS
enforces the ceiling with a `proc_pid_rusage` physical-footprint watchdog + process-group SIGKILL;
Linux classifies the pod-cgroup/VRAM OOM exit. A new `unboundedEngineSpawnViolations` lint keeps new
engine-spawn call sites off the raw primitives.
...
- **Phase 0 Sprint 0.15** — the `bounded_inference_memory.md` doctrine doc, its docs-lint registration
  ...
  `cabal build all`.
- **Phase 4 Sprint 4.30** — the grant-gated capped-engine kernel: `admitModelMemory` returns
  `Either InferenceError MemoryGrant`, the only engine-spawn path (`Infernix.Runtime.CappedEngine.withCappedEngine`)
  requires the grant and bounds resident memory to the admitted `MemoryCeiling`, with the macOS
  `proc_pid_rusage` watchdog and the Linux OOM-exit classifier. **Code-side closed (2026-07-21).**
- **Phase 4 Sprint 4.31** — the checked `HostMemoryPartition`, the required `ModelMemoryFootprint`, and
  the budget-enforcer split dropping `UnenforcedMemoryBudget`. **Code-side closed (2026-07-21)** (built on Sprint 4.30).
...
The Phase 4/6 code is **code-side closed** (machine-independent gate set GREEN on this Apple host,
2026-07-21); the remaining residual is the single-accelerator (apple-silicon) plus `linux-cpu`
full-suite behavioral sign-off — an over-capacity catalog completing with zero host OOM-kill and every
over-budget row cleanly typed-rejected as `ModelMemoryLimitExceeded` — which is
[Wave W](cohort-validation-waves.md), **Open (Stage 1 GREEN; Stage 2 pending)**. The superseded
proof-free admission, the raw engine spawns, the bare-`Int` footprint, the `UnenforcedMemoryBudget` arm,
and the hard-coded `appleHostReserveMib = 3072` reserve are recorded in
[legacy-tracking-for-deletion.md](legacy-tracking-for-deletion.md).
...
`infernix docs check`, `poetry run check-code`, web unit `83/83`); the remaining residual is the
single-accelerator (apple-silicon) plus `linux-cpu` behavioral re-run — a retained second `cluster up`
warming the cache without the "11/16" stall — which rides [Wave W](cohort-validation-waves.md). The
...
Constraint 0): the doctrine + governance land now, and the enforcing code is `Planned`.
...
- **Phase 0 Sprint 0.16** — the doctrine extension in `managed_state_transitions.md`, the new
  ...
  + `cabal build all`).
- **Phase 2 Sprint 2.15** (`Planned`) — the model half: the `ClusterOwner` field, the `ClusterMutating`
  lifecycle position, its fail-closed versioned persistence, and reconcile-on-next-`cluster up`.
- **Phase 6 Sprint 6.43** (`Planned`) — the harness half: the evidence-gated seizure (fail closed on an
  `OperatorOwned` cluster), the chaos-mutation `ClusterMutating` transitions, and the crash-safe
  `withTestHarnessConfig` backup reconcile.

The single-accelerator (apple-silicon) plus `linux-cpu` behavioral proof, once the code lands, is
[Wave X](cohort-validation-waves.md). The superseded ownerless `ClusterReady`-as-idle, the unconditional
...
|-------|------|--------|----------|
| 0 | Documentation and Governance | Done — Cluster-Ownership & Mutation-Position reopen (Sprint 0.16 doctrine + governance mirror landed 2026-07-23, doc-only/machine-independent, `Done` on `infernix lint docs` + `docs check` + `cabal build all`; enforcing Phase 2 Sprint 2.15 + Phase 6 Sprint 6.43 code is `Planned`, [Wave X](cohort-validation-waves.md) pending); Memory-safety-by-construction reopen (Sprint 0.15 doc + governance mirror landed 2026-07-21, doc-only/machine-independent, Done on `infernix lint docs` + `docs check` + `cabal build all`; enforcing Phase 4/6 code is code-side closed 2026-07-21, [Wave W](cohort-validation-waves.md) Stage 2 pending); Bounded-Command Application & Bounded-HTTP reopen (Sprint 0.14 code-side closed 2026-07-19, cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20)); Managed-State-Transition Doctrine reopen (Sprint 0.13 code-side closed 2026-07-16, cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20)); prior Done — reopened and re-closed (Sprints 0.1-0.12 done; Sprint 0.11 reconciled the governed docs — README matrix, `model_catalog`, `testing_strategy`, `python_policy`, realness doctrine — to the code-enforced realness invariant in lockstep with the Phase 4 catalog change, and Sprint 0.12 added the machine-independent realness lint enforcement (Python `check-code` AST + Haskell `realnessFabricationViolations`, scope extended per accelerator phase); validated 2026-06-23, machine-independent) | [phase-0-documentation-and-governance.md](phase-0-documentation-and-governance.md) |
| 1 | Repository and Control-Plane Foundation | Active — Observable-Readiness reopen (Sprint 1.18 — the tri-state `PollOutcome` readiness kernel + `awaitReadinessObservable`, `awaitReadiness` preserved as a behaviour-identical lift) is **code-side closed (2026-07-22)** on the machine-independent gate set, with the apple-silicon plus `linux-cpu` behavioral re-run pending [Wave W](cohort-validation-waves.md); prior Done — Bounded-Command Application & Bounded-HTTP reopen (Sprint 1.17 code-side closed 2026-07-19, cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20)); Managed-State-Transition Doctrine reopen (Sprint 1.16 code-side closed 2026-07-16, cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20)); prior Done — reopened and re-closed (Sprints 1.1-1.14 remain closed for the scaffold/topology/materialization-lane foundation; Sprint 1.15 is closed for real Apple native runner materialization and native snapshot hydration. Apple Stage 2 integration plus focused routed Playwright are green, and the paired `linux-cpu` full gate closed on 2026-06-29 with rebuilt image `sha256:f243cf3a7c5199746321bffba87639e30fda959e2be80c7d3b15a413fb9e9ca8`: `./bootstrap/linux-cpu.sh test` passed style, Python `check-code`, unit, web `71/71`, full integration with all real `linux-cpu` model outputs plus the HA/chaos tail, and routed Playwright `9/9`.) | [phase-1-repository-and-control-plane-foundation.md](phase-1-repository-and-control-plane-foundation.md) |
| 2 | Kind Cluster Storage and Lifecycle | Active — Cluster-Ownership & Mutation-Position reopen (Sprint 2.15 — the typed `ClusterOwner`, the `ClusterMutating` lifecycle position, its fail-closed persistence, and reconcile-on-next-`cluster up`) is **documentation-first**: the doctrine + governance landed (Phase 0 Sprint 0.16, `Done`), and the enforcing code is `Planned`, tracked as the [Wave X](cohort-validation-waves.md) residual; prior Done — Managed-State-Transition Doctrine reopen (Sprint 2.14 code-side closed 2026-07-16: typed ClusterLifecycle machine, fail-closed versioned aeson persistence, and WriterQuiesced lease-gated teardown; cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20)); prior Done (Sprints 2.10-2.13 lifecycle, retained-state, bootstrap-boundary, and host-manifest closure validated by Apple Wave A and CUDA Linux Wave C) | [phase-2-kind-cluster-storage-and-lifecycle.md](phase-2-kind-cluster-storage-and-lifecycle.md) |
...
| 4 | Inference Service and Durable Runtime | Active — Memory-safety-by-construction reopen (2026-07-21): Sprint 4.30 (grant-gated capped-engine kernel — `admitModelMemory` returns `Either InferenceError MemoryGrant`, resident memory bounded to the admitted `MemoryCeiling` by `Infernix.Runtime.CappedEngine.withCappedEngine`, macOS `proc_pid_rusage` watchdog + Linux OOM-exit classifier, raw engine spawns retired) and Sprint 4.31 (checked `HostMemoryPartition`, required `ModelMemoryFootprint`, budget-enforcer split dropping `UnenforcedMemoryBudget`) are **code-side closed (2026-07-21)** — the prior proof-free `Maybe` admission and raw/unbounded engine spawn that made a host OOM representable are gone; machine-independent gate set GREEN — with the behavioral cohort sign-off pending [Wave W](cohort-validation-waves.md) (Open, Stage 1 GREEN / Stage 2 pending). Prior Done — Bounded-Command Application & Bounded-HTTP reopen (Sprint 4.29 code-side closed 2026-07-19: UA header + `Retry-After`-honoring bounded-redelivery `DownloadOutcome` fold + integrity-witnessed `PayloadVerified` byte-length check; cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20)); Managed-State-Transition Doctrine reopen (Sprint 4.28 code-side closed 2026-07-16: PayloadVerified-gated `.ready` commit, typed awaitModelBootstrapReady, native-runner HOME/TMPDIR; cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20)); prior Done — Sprint 4.27 is closed for typed runtime resource admission: pure `InferenceMemoryBudget` / `InferenceError` types replace the Apple-only integer budget, config-time over-budget fail-fast, hardcoded floor, and stringly failure payload. Runtime admission rejects only the oversized request with typed `ModelMemoryLimitExceeded { requiredMib, availableMib, resource, source }`; smaller configured models keep running. Wave T closed on 2026-07-12 with `linux-cpu` plus selected `linux-gpu` full-suite evidence. Earlier Sprints 4.22/4.25/4.26 remain closed by Waves P/R/S for their original scopes. | [phase-4-inference-service-and-durable-runtime.md](phase-4-inference-service-and-durable-runtime.md) |
...
| 6 | Validation, E2E, and HA Hardening | Active — Cluster-Ownership & Mutation-Position reopen (Sprint 6.43 — the evidence-gated harness seizure, chaos-mutation `ClusterMutating` transitions, and the crash-safe `withTestHarnessConfig` swap) is **documentation-first**: the doctrine + governance landed (Phase 0 Sprint 0.16, `Done`), and the enforcing code is `Planned`, tracked as the [Wave X](cohort-validation-waves.md) residual. Memory-safety-by-construction reopen (2026-07-21): Sprint 6.42 (`unboundedEngineSpawnViolations` capability-gating lint keeping new engine-spawn call sites off the raw process primitives and on the Phase 4 Sprint 4.30 grant-gated capped-engine kernel) is **code-side closed (2026-07-21)** — wired into `checkSourceReadability`, negative-tested, machine-independent gate set GREEN — with the behavioral cohort sign-off pending [Wave W](cohort-validation-waves.md) (Open, Stage 1 GREEN / Stage 2 pending). Prior Done — Bounded-Command Application & Bounded-HTTP reopen (Sprint 6.40 code-side closed 2026-07-19: `unboundedExecViolations` + `unboundedHttpViolations` capability-gating lints, cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20); Sprint 6.41 Done — ProcessMonitor retired + `retryCommandOutput`/6 consumers + eager-cache barrier + the full twelve-wait individual bounded-wait migration onto `awaitReadiness`/`budgetDeadline` + the `threadDelayViolations` lint gate, all code-side closed 2026-07-19 (machine-independent, adversarially reviewed), cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20)); Managed-State-Transition Doctrine reopen (Sprint 6.39 code-side closed 2026-07-16: capability-gating lint (raw-rm / empty-env) + routed managed-transition evidence assertion; cohort gate closed by [Wave V](cohort-validation-waves.md) (2026-07-20)); prior Done — Sprint 6.38 is closed for typed resource-admission validation across Apple unified host RAM, Linux CPU pod memory, and Linux GPU VRAM. Unit/static coverage proves no config-wide startup failure for one oversized model, enforced zero budget behavior on Apple over-pledge, constructor-based `ModelMemoryLimitExceeded` classification, and substrate budget-source selection; Wave T closed on 2026-07-12 with `linux-cpu` plus selected `linux-gpu` full live integration/e2e evidence. Earlier Sprints 6.35/6.36/6.37 remain closed by Waves P/R/S for their original scopes. | [phase-6-validation-e2e-and-ha-hardening.md](phase-6-validation-e2e-and-ha-hardening.md) |
