...
2026-07-04. The later 2026-07-06 Wave Q review re-closed Phase 9 (access control and monitoring) on
both `apple-silicon` and `linux-cpu`, and reopened Phases 4 and 6 for the matrix substrate-accuracy
hardening (Sprints 4.25 and 6.36) plus the 2026-07-07 apple-silicon inference RAM-safety gap
(Sprints 4.26 and 6.37). [Wave R](cohort-validation-waves.md) (2026-07-08) **code-side closes all
four** on this Apple host with the full machine-independent gate set green: an explicit per-model
`modelRamFootprintMib` and per-substrate `inferenceRamBudgetMib` make on-host apple-silicon inference
RAM-safe by construction (host-computed budget = physical RAM − colima pledge − reserve, config-time
hard-fail, and serialized-critical-section admission control that rejects an over-budget model as a
clean `status=failed` instead of an OS OOM-kill); the matrix-accuracy fixes relabel/document the CUDA
lanes honestly; and the validation suites gain a real-inline-text marker, a catalog-completeness
guard, and a memory-exhaustion classification. The only open residual is single-accelerator sign-off:
the full Apple per-model never-OOM rerun (paired 4.26/6.37) plus the CUDA GPU-accuracy `linux-gpu`
rows that need a CUDA Linux host.
...
| 4 | Inference Service and Durable Runtime | Active — Sprints 4.25 (matrix substrate-accuracy) and 4.26 (apple-silicon inference RAM admission) are code-side complete and machine-independent-validated under [Wave R](cohort-validation-waves.md) (2026-07-08), and the **full Apple cohort is GREEN**: `test integration` drove all 16 apple catalog models to `status=completed` with **zero OS OOM-kill** (proving the per-model RAM-footprint + host-computed budget + config-time hard-fail + serialized admission control), the routed `test e2e` per-model browser matrix passed (all 16 models real output through the UI), and the cohort run also fixed a Dockerfile stage-zero host-manifest schema drift the machine-independent gates could not catch. Remaining sign-off: the `linux-cpu` full-suite plus the CUDA GPU-accuracy `linux-gpu` rows (need a CUDA host). The prior Wave O MT3 catalog replacement (Sprint 4.22) is **closed** — proven by Wave P (2026-07-04). Sprints 4.1-4.21/4.23/4.24 remain closed for their original scopes. | [phase-4-inference-service-and-durable-runtime.md](phase-4-inference-service-and-durable-runtime.md) |
...
| 6 | Validation, E2E, and HA Hardening | Active — Sprints 6.36 (real-output + matrix validation hardening) and 6.37 (apple-silicon memory-bounded validation lane, unblocked by Phase 4 Sprint 4.26) are code-side complete and machine-independent-validated under [Wave R](cohort-validation-waves.md) (2026-07-08), and the **full Apple cohort is GREEN**: the 16-model per-model `test integration` drove all rows to `status=completed` with zero OS OOM-kill (the `classifyAppleMemoryBoundedResult` classification), and `test e2e` ran the routed per-model browser matrix (the `Chat.purs` `data-inline-output` real-text marker + picker catalog-completeness guard) plus the Phase 9 RBAC/dashboard specs at 13/15 (the 2 failures are timing flakes in Phase 7 specs untouched by these sprints). Remaining routed sign-off: the `linux-cpu` full-suite (plus the optional cohort-authored union gate, row-14 xfail, multi-artifact, and HA/chaos footprint case), with the CUDA-only accuracy `linux-gpu` rows the named residual. The prior Wave O expanded-MT3 integration/e2e gate (Sprint 6.35) is **closed** — proven by Wave P (2026-07-04). Sprints 6.1-6.34 remain closed for their original scopes. | [phase-6-validation-e2e-and-ha-hardening.md](phase-6-validation-e2e-and-ha-hardening.md) |
