...
Phases 0 through 3, 5, 7, 8 and 9 are `Done`. Phase 8 closed on the CUDA Linux wave described below.
Phases 1 and 2 closed on 2026-08-16 under
...
**What remains, and why.** Every open item is now the same kind: a cohort gate whose code is
complete. The Bounded Engine Launch architecture landed in numerical order — a model's memory
requirement is derived from its artifact rather than authored as a per-family constant, the
requirement is resource-indexed because host and device are different formulas, three sampling loops
became one over the field the ceiling actually charges, and enforcement moved from sampling an engine
after it allocates to installing a kernel ceiling before it does on the lane that can install one.
Its host and device halves are separate contracts on separate accelerators, which is why they remain
separate waves.

[Wave Z](cohort-validation-waves.md) closed on its fourteenth attempt: both the `linux-gpu` and the
paired `linux-cpu` lane exited 0 against one frozen source, each running real model output, so the
nine sprints that named it have their behavioral evidence. What remains is two items, and neither is
this repository's code being unfinished.

| Open item | Blocker |
|-----------|---------|
| Phase 6 Sprint 6.51 | [Wave AD](cohort-validation-waves.md) — the calibration pass and the device-peak remeasurement, neither of which an ordinary run performs. The wave's closing run narrowed the blocker to one named defect: the device backstop watches the admitted grant while a framework adapter is sized by the lane's device budget |
| Phase 4 Sprints 4.37-4.42 | the Bounded Engine Launch host half's cohort, on the Apple accelerator plus `linux-cpu` ([Wave AC](cohort-validation-waves.md)). No Apple Silicon host is present on the machine this work was done on, and Section Q forbids substituting the other accelerator |

Under Section C, an earlier phase reopening does not revert a later one: each phase's `Done` was
earned against its own deliverables and its own cohort. Historical wave evidence remains source-scoped
and does not substitute for a current owning-phase gate.
...
Phases 0 through 3, 5, 7, 8 and 9 are `Done` with no open sprint. Every remaining open sprint is
code-side closed and holds only a cohort gate: Phase 6 has one on [Wave AD](cohort-validation-waves.md),
and Phase 4 has the six that make up the Bounded Engine Launch host half on the Apple wave; its Sprints 8.11 and 8.12 closed on 2026-08-18, the latter on a live two-machine `linux-cpu`
...
cannot reach admission at all. The requirement admission compares against is presently an authored
per-family constant admitted against every resource a placement uses; Phase 4's open sprints replace it
with a requirement derived from the artifact's own bytes and indexed by physical resource, and move
enforcement from sampling an engine after it allocates to installing a ceiling before it does. Budgets carry typed `InferenceMemoryBudget` semantics, admission
...
| 4 | Active | Sprints 4.43 and 4.44 are `Done`: [Wave Z](cohort-validation-waves.md) found both and closed both, on both lanes against one frozen source. Sprint 4.43 installs the greater of the artifact-derived requirement and the engine's own bounded pre-flight projection and records which quantities produced the value; Sprint 4.44 makes a kernel refusal at that ceiling a typed outcome rather than an unclassified engine exit. Between them the wave also corrected four defects it surfaced: detection now watches the quantity prevention installs, the probe is not bounded by the quantity it exists to correct, a framework binding installs the lane's own per-execution budget rather than a checkpoint's tensor sum, and an adapter's acknowledgement rides its error responses so a conformance check cannot mask the engine's own failure. Sprints 4.37-4.42 are the Bounded Engine Launch host half and are all code-side closed, holding one shared [Wave AC](cohort-validation-waves.md) residual: the derived, resource-indexed memory requirement, one sampling kernel over the field the ceiling charges, the installed ceiling, and the execution shape reaching the engine. Sprint 4.37 landed first and deliberately alone, because it makes a breach name the resource it breached, and a cohort that cannot say which resource it exceeded cannot be diagnosed from its own output. Sprint 4.39's derivation deliberately narrows the admissible catalog to the rows whose checkpoint header the two landed readers understand; every other row is retained as an explicit unavailable placement naming the artifact family whose reader is absent. Every earlier sprint is closed. Sprints 4.31, 4.32, 4.34, and 4.35 closed together on 2026-08-17 against one frozen source identity validated on `apple-silicon` plus the paired `linux-cpu` lane, recorded in [Wave Y](cohort-validation-waves.md). That cohort also surfaced and closed two defects of its own: a native runner could reach its engine with an unhydrated model cache, and the integration suite's routed probes were single-shot behind a retry predicate that classified on text it never receives. Sprint 4.36 is `Done` by supersession and re-home into Phase 1 Sprint 1.23; the broker-side member claim is owned by Phase 8 Sprint 8.12. |
...
| 6 | Active | Sprints 6.44, 6.50, and 6.52 are `Done` on the closed [Wave Z](cohort-validation-waves.md) cohort — both lanes exited 0 against one frozen source, the live NVIDIA and live CUDA ceiling-breach assertions both ran in-cohort, and a native artifact reached real model output under an injected-driver loader cache. Sprint 6.51 is the phase's only open item: its ordinary cohort half is met and a framework row reached a live device with a measured typed `gpu-vram` breach, but [Wave AD](cohort-validation-waves.md)'s calibration pass and device-peak remeasurement are unmade, and that run named the defect they are blocked behind — the device backstop watches the admitted grant while the adapter is sized by the lane's device budget. Sprint 6.51 is the Bounded Engine Launch device half — admission, arena sizing, and a sampled backstop rather than a kernel bound, because no kernel mechanism bounds device memory on any lane; its arena is now the admitted quantity over the observed device envelope, and the card's free memory is observed inside the serialized execution region immediately before the engine starts. Every other sprint is `Done`. Sprints 6.37, 6.43, 6.45, 6.46, 6.47, 6.48, and 6.49 closed together on 2026-08-17 against one frozen source identity validated on `apple-silicon` plus `linux-cpu`, recorded in [Wave Y](cohort-validation-waves.md); that identity added Sprint 6.46's last reopened deliverable, the point-of-use bound observation on the production spawn path. Sprint 6.44 is code-side closed and holds a **supported-lane validation blocker**: its `linux-gpu` behavioral cohort needs a CUDA-capable Linux host, which Section Q forbids substituting the other accelerator for. |
