...
RBAC/STS/dashboard surface on both `apple-silicon` and `linux-cpu`, and reopened Phases 4 and 6 for the matrix substrate-accuracy
...
`linux-gpu` full-suite residuals. A later UAT pass then surfaced an unresolved authentication issue
in Phase 9's admin-vs-user access domain (repo-root `notes.txt`); Phase 9 is therefore `Active` again
with a named UAT auth residual until that issue is diagnosed and re-validated.
