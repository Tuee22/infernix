...
| 4 | Inference Service and Durable Runtime | Active (real per-family inference code-side closure **complete** across Sprints 4.1/4.2/4.3/4.7/4.8/4.10/4.11/4.12/4.14/4.15 plus new Sprints 4.16 (per-engine isolated framework venvs) and 4.17 foundation (per-engine engine images — the control-plane/per-engine Dockerfile split to a 22.4 GB slim image plus per-engine images, and the `Infernix.Models` engine→name/topic/image mapping) — `ResultFamily` + object-ref plumbing, real native-binary + Python-adapter dispatch behind lazy guarded imports, `buildPayload` family routing, fail-fast-on-unsupported, per-engine `python/engines/<engine>` venvs that keep `check-code` machine-independent, and the per-engine image split — proven on the present CUDA Linux host (Stage 1: `cabal build all`, `cabal test infernix-unit`, `cabal test infernix-haskell-style`, `infernix lint proto/files/docs/chart`, `poetry run check-code`); an early Stage 2 cohort proof — torch-cu128 plus a real Qwen2.5 generation on the RTX 5090 through the transformers adapter — already runs on this host, but real-engine output for every row is the full Stage 2 cohort batch in [Wave I](cohort-validation-waves.md), with Omnizart/MT3 named cohort residuals and Sprint 4.17's cluster-side batch routing/chart/lifecycle wiring deferred to the same Wave I live-iteration; the prior Sprints 4.1-4.14 closed with mounted `ClusterConfig` / `SecretsConfig` runtime path validated by Apple Wave A and CUDA Linux Wave C) | [phase-4-inference-service-and-durable-runtime.md](phase-4-inference-service-and-durable-runtime.md) |
