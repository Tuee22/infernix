...
| X | Apple Silicon + Linux CPU | **Cluster-Ownership & Mutation-Position reopen (2026-07-23) — closed 2026-07-24.** An externally-killed `infernix test all` exposed a DSL smell: `ClusterState` had no owner and `ClusterLifecycle` had no mutating position, so a test-mutated cluster (drained node / over-scaled deployment) was the same term as an operator's idle `ClusterReady` and a SIGKILLed run persisted a false `steady-state`; and `runClusterOwnedValidation`'s unconditional `clusterDown` over the shared operator cluster identity (`findRepoRoot`) let even a clean run destroy an operator's cluster. The doctrine + governance landed first — **Phase 0 Sprint 0.16** (doc-only, `Done`): the ownership + mutation-position law in `managed_state_transitions.md`, the new non-negotiable rule in the three-way `README.md`/`AGENTS.md`/`CLAUDE.md` + `assistant_workflow.md` mirror, the `documentation_standards.md` cluster-lifecycle Update Rule, and the operator / test-harness / persistence doc reconciliation (`infernix lint docs` + `docs check` + `cabal build all` green). The enforcing code is now **implemented and Stage 1 (the machine-independent gate set) is GREEN (2026-07-23)**: `ClusterOwner` + `ClusterMutating` + fail-closed persistence (pre-migration documents decode to the safe default `OperatorOwned`) + reconcile-on-next-`cluster up` (**Phase 2 Sprint 2.15**), and the evidence-gated harness seizure (`seizeHarnessClusterSlot` over a hidden-constructor `ClusterTeardownAuthority`) + chaos-mutation `ClusterMutating` transitions (`withPersistedClusterMutation`) + crash-safe `withTestHarnessConfig` swap (`reconcileLeftoverHarnessBackup`) (**Phase 6 Sprint 6.43**). Stage 1 gates: `cabal build all` `-Wall -Werror`, `cabal test infernix-unit` (owner round-trip + pre-migration `OperatorOwned` default + `ClusterMutating` renders dirty + seizure fail-closed matrix + `.harness-backup` restore), `cabal test infernix-haskell-style`, `infernix lint files/docs/proto/chart`, `infernix docs check`, the web unit suite, and `poetry run check-code` all pass on this Apple host. Stage 2 (behavioral) is the single-accelerator (apple-silicon) plus `linux-cpu` full-suite proof: a killed test's cluster reports mutation-incomplete and reconciles on the next `cluster up`, an operator's cluster is never destroyed, and a leftover `.harness-backup` is restored on the next `test`. | Closed — Stage 2 behavioral proof GREEN on both lanes against frozen workload image `sha256-bcf88c23fda211a4b5f3701c1c1c66ab223462f40d709be795e8f7b2d44ccee0` (2026-07-24). **apple-silicon**: `infernix test integration` against a running (migrated `OperatorOwned`) operator cluster **failed closed loud** (`refusing to seize the cluster slot: an operator cluster is up (owner=operator)…`), restoring the operator config byte-identical and leaving the cluster intact — the harness can no longer destroy an operator's cluster; `cluster status` reported `clusterOwner: operator` (a pre-migration ownerless document decoded to the safe default); and the full `infernix test all` (integration PASS + routed Playwright 16/16) exercised the `HarnessOwned` bring-up/seizure lifecycle. **linux-cpu**: clean `infernix test all` — the node-drain and deployment-over-scale chaos mutations ran inside `withPersistedClusterMutation` (persisting `ClusterMutating` across the real cordon/drain and over-scale, restoring `ClusterReady` after) with integration PASS and routed Playwright 16/16, and the crash-safe `reconcileLeftoverHarnessBackup` restored a leftover harness config backup on container entry. | 2026-07-24 |
