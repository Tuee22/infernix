...
> **Current execution — Phase 1 Sprints 1.20–1.25 (2026-08-13).** Phase 0 is Done through Sprint
> 0.24: the plan-standards scans report zero against the corpus and run inside `infernix test lint`.
> Phase 1's machine-independent gate set is GREEN on the current source identity — governed Apple
> build, `infernix test lint`, `infernix test unit` (compile-fail, artifact transaction, Apple
> materializer, capped observer, execution-plan, unit), the standalone `lint files`, `lint docs`,
> `lint chart`, `lint proto`, `lint plan`, and `docs check` commands, and a repo-wide
> `git diff --check`, each exiting 0.
>
> `./.build/infernix internal materialize-metal-engines` is RED on that identity, at a defect the
> earlier failures had masked. Three distinct failures were found and the first two corrected: the
> JavaCPP cache-link normalizer built its replacement with `makeRelative`, which never ascends and
> so could not rewrite the sibling-jar link it exists for, and decided containment on uncollapsed
> bytes; and the Audiveris installed smoke ran the bundled JVM's `java -version` instead of the
> inference target's own classpath and main class, so it could never emit the seven-field banner its
> parser is bound to. Both are corrected, unit-proven, and exercised in production — the run now
> reaches and passes the Audiveris smoke. The third is open: the smoke loads Tesseract through
> JavaCPP, which reabsolutizes a cache link inside the sealed candidate after the payload was
> hashed, and the transaction refuses the changed candidate. Custody after that refusal is clean —
> six complete roots, no candidate, temporary, previous, or mount residue, `jvm-native` absent, no
> claimant.
>
> `./.build/infernix internal validate-darwin-build-memory` is GREEN on this identity: 560
> one-second samples, a 1860537392-byte sampled peak aggregate physical footprint against the 8192
> MiB account, both invocations exiting 0. It carries its own caveat — Darwin enforces no aggregate
> or address-space ceiling for this lane, fixed-interval sampling can miss a transient peak, and
> processes outside the measured Cabal group are not bounded. It does not discharge Sprint 1.21's
> residual, because the identity Phase 1 closes on is not yet this one: the materializer defect
> above must land first, and the measurement belongs to whatever identity that correction produces.
>
...
The historical closed rows remain unchanged. The status cells for Waves Y, Z, AA, and AB below
retain their recorded evidence and current disposition: AA is now Done, while Y/Z/AB remain open.
Current scheduling follows the restored numerical order:
Phase 1 is Active on the shared Phase 0 `Subprocess`/`Spec` identity, whose build, whole aggregate
lint, full unit, standalone lint/docs, and repo-wide diff are GREEN. Its first materializer retry is
INVALIDATED by an external Cabal/GHC claimant outside the owned tree, never RED or GREEN, and makes
no behavioral claim. Earlier
Audiveris build GREEN/runtime RED/lint INVALIDATED receipts remain identity-scoped. Custody has no
claimants/active roots, exactly six complete roots/six top-level engine directories, zero
sibling/tmp/previous/download/Audiveris residue, absent `jvm-native`, and retained quarantine. Wave
AA Stage 1 plus the Darwin validator are already GREEN and are not rerun. At the invalidated-attempt
checkpoint, the prior schedule was to retry the same exact materializer after new Plan Quiet and a
fresh strict quiet window, before Wave Y. The later uncontested 2026-08-12 materializer supersedes
that next-gate boundary: it is genuine unsafe-link RED with no seven markers and GREEN final custody.
Its cache-local JavaCPP-link correction is SOURCE-STABLE/unbuilt/unrun after all review fixes, with
final current-tree review CLEAN/no High or Medium and repo-wide diff-check GREEN. Run the complete
governed gates before retry; specialized validators and
Wave Y remain blocked. Wave AA remains GREEN/Done/no-rerun.
Phase 2 and each later phase remain blocked by
their predecessor, with Wave Z/AB queued accordingly.
The Wave Y row's historical final `Diagnose/correct/review` scheduling clause is superseded by this
SOURCE-STABLE correction checkpoint; its recorded evidence and every other residual remain exact.
...
## Wave Y: Phase 1, Phase 2, Then Typed Execution Plan Apple/CPU Sign-Off (Active)
...
| 1 | Wave AA Stage 1 plus `validate-darwin-build-memory` are GREEN/no-rerun. Wave Y alone remains queued. Shared current Phase 0 `Subprocess`/`Spec` identity build, whole aggregate lint, full unit, standalone lint/docs, and repo-wide diff GREEN. First materializer retry remains INVALIDATED by external Cabal/GHC overlap outside the owned tree, never RED/GREEN, as chronology. The uncontested 2026-08-12 materializer is genuine unsafe-link RED with no seven markers and GREEN 13:33:40 custody. Its specialized cache-local JavaCPP-link correction is SOURCE-STABLE/unbuilt/unrun after all review fixes, with final current-tree review CLEAN/no High or Medium and repo-wide diff-check GREEN. Earlier Audiveris build GREEN/runtime RED/lint INVALIDATED receipts retain exact scope. No claimants/active roots; exactly six complete roots/six top-level engine dirs, zero sibling/tmp/previous/download/Audiveris residue, `jvm-native` absent, quarantine retained. Wave Y requires schema-complete Apple and paired `linux-cpu` attestations for `llm-smollm2-safetensors`, `audio-demucs-htdemucs`, `audio-open-unmix`, `music-mt3-infer`, `music-mr-mt3`, `music-omnizart`, and `audio-bark-small`. Independent normalization and final Wave Y/AA closure-sequencing reviews CLEAN/no High/Medium. Governed build, whole lint, full unit, standalone lint/docs, docs check, and repo-wide diff precede materializer retry under fresh admission; specialized validators and cohort work remain blocked |
