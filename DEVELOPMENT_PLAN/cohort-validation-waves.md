...
> **Apple Silicon and `linux-cpu` evidence reset (2026-08-08).** Every Apple Silicon and `linux-cpu`
...
> sprint, or wave.
>
> The reset is not precautionary. Three hard Darwin failures have already been found by audit and by
...
> renderer actually produces, which blocks `test lint` **and** `cluster up`. A survey of 10 of the
> 82 changed files found 3 will-break and 7 might-break blockers. **72 files remain unaudited**, so
> the blocker inventory is a floor.
>
> **Re-established since the reset**: the library and every test suite compile on Darwin under
> `-Wall -Werror`; `cabal test infernix-unit`, `cabal test infernix-haskell-style`, and
> `cabal test infernix-compile-fail` are green with no local scaffold; `infernix lint docs|files`
> pass; `infernix test lint` reaches and passes the `infernix-haskell-style` suite through the real
> CLI path. **Not re-established**: `infernix test all` and `cluster up` on either lane, and
> therefore `bootstrap/apple-silicon.sh` and `bootstrap/linux-cpu.sh` full lifecycles. The
> corrections are owned by Phase 1 Sprints 1.20/1.21/1.22, Phase 4, and Phase 6, and the cohort
> residual belongs to [Wave Y](#wave-table).
...
| 1 | Sprints 1.1-1.19 retain their recorded closure. Sprint 1.20 is code-side closed on 2026-08-02: descriptor-retained writer effects, self-contained installed targets, complete loader closure, closed invocation languages, exact helper-owned cleanup, full machine-independent proof, and settled-source review with no High or Medium residual are complete. Exact image `sha256:51292f6f3d98560b383a4ab5cc8a1807aa5388fa5cc0ba8c99b305d90ba9ff67` passed the paired `linux-cpu` full suite. Phase 1 remains Active only for validation-only Wave Y Apple evidence. | Wave Y Apple corrected-root rematerialization, production Audiveris cancellation, source-runtime-unavailable installed smokes, routed runtime loading, and full `integration`/`e2e`/`all` remain on Apple hardware. | GREEN on 2026-08-02: exact-source `./bootstrap/linux-cpu.sh test`, including integration, 16/16 Playwright, full catalog per-model browser matrix, restoration, and final teardown; no CUDA claim is added |
...
| 4 | Sprints 4.1-4.31 retain their recorded narrower closure. Sprint 4.32 is code-side closed on 2026-08-02: fixed bounded `/usr/bin/top` plus `/usr/bin/footprint` Apple observation, verified Linux process-group RSS watchdog, opaque plan-owned single-flight authority, fail-closed sampler loss, and live Linux breach regression are implemented. Wave W's direct-FFI sampler evidence remains superseded. | Original contract closed in Wave A; current Sprint 4.32 Apple observer startup plus live ceiling-breach/survival and full-suite evidence remain the only Phase 4 work | GREEN 2026-08-02 on exact image `sha256:dfc0e2b6251e2d7ed74712253e06d2f9fbc60b649ac162b44dac030aca43a979`: complete `./bootstrap/linux-cpu.sh test`, integration PASS, routed Playwright `16/16`, and clean teardown |
