...
> unrewritten for their narrower recorded scope, per the convention above. The bound itself is
> proved by the typed gates, the negative-compilation fixtures, and the adversarial breach fixtures
> named in [../documents/architecture/bounded_host_memory.md](../documents/architecture/bounded_host_memory.md),
> and the correction is owned by Phase 4 Sprint 4.33.
...
> runs first. Wave AA Stage 1 plus the
> closed Darwin validator are already GREEN and require no rerun.
...
lane bundles must name the same frozen source identity. Wave AA is separate and already GREEN for
Sprint 1.21 Stage 1 plus `internal validate-darwin-build-memory`; Wave Y neither reopens nor reruns it.
...
| 8 | Validation-only work and Sprint 8.11 design blocker remain; Wave AB remains queued |
...
| 8 | Sprints 8.1–8.8 are `Done`; Sprints 8.9 and 8.10 are Active — Validation Only; Sprint 8.11 is Blocked pending a real per-machine Linux identity/configuration surface | No new Apple-only claim closes Sprint 8.11 without the missing per-machine contract home | Sprints 8.9/8.10 consume the current `linux-gpu` plus `linux-cpu` rebuild; Sprint 8.11 remains unstarted pending the design and Wave AB proof |
