...
|----------|------------------------|------------------------|
| `src/Infernix/Edge.hs` | Routing moves to Envoy Gateway API (`Gateway/infernix-edge` plus HTTPRoutes); the Haskell unified edge proxy is no longer part of the supported path | Phase 3, Sprint 3.5 |
| `src/Infernix/Gateway.hs` | Per-backend portal proxying moves to HTTPRoute manifests with `URLRewrite` filters; the Haskell platform gateways are no longer part of the supported path | Phase 3, Sprint 3.8 |
| `src/Infernix/HttpProxy.hs` | Shared proxy-helper code is orphaned once `Edge.hs` and `Gateway.hs` are removed | Phase 3, Sprint 3.5 |
| `infernix edge` and `infernix gateway harbor|minio|pulsar` CLI subcommands in `src/Infernix/CLI.hs` | The supported routing surface is the Helm-installed Envoy Gateway controller plus HTTPRoute manifests; the Haskell entrypoints are removed | Phase 3, Sprint 3.5; Phase 3, Sprint 3.8 |
| `chart/templates/deployment-edge.yaml`, `chart/templates/service-edge.yaml`, `chart/templates/edge-configmap.yaml` | Replaced by `chart/templates/gateway.yaml`, `gatewayclass.yaml`, and the `httproutes/` set under the new doctrine | Phase 3, Sprint 3.5 |
| Gateway entries in `chart/templates/workloads-platform-portals.yaml` (`infernix-harbor-gateway`, `infernix-minio-gateway`, `infernix-pulsar-gateway`) | Replaced by HTTPRoute manifests with `URLRewrite` filters; no per-backend Haskell gateway pods remain | Phase 3, Sprint 3.8 |
| Harbor admin Basic-auth header injection in `src/Infernix/Gateway.hs` (`harborAuthorizationHeader` + `INFERNIX_HARBOR_ADMIN_USER`/`INFERNIX_HARBOR_ADMIN_PASSWORD` env vars) | The demo cluster runs locally and applies no auth filters; the credential injection is dropped without replacement | Phase 3, Sprint 3.8 |
| `docker/infernix.Dockerfile` | The launcher image folds into the per-substrate container; the substrate image plays launcher, in-cluster workload, and Playwright executor | Phase 4, Sprint 4.9 |
| `docker/service.Dockerfile` | The in-cluster service image folds into the per-substrate container | Phase 4, Sprint 4.9 |
| `docker/vllm-python.Dockerfile`, `docker/transformers-python.Dockerfile`, `docker/diffusers-python.Dockerfile`, `docker/pytorch-python.Dockerfile`, `docker/tensorflow-python.Dockerfile`, `docker/jax-python.Dockerfile` | Engine deps move into the per-substrate container at image build time; per-engine Dockerfiles are no longer part of the supported path | Phase 4, Sprint 4.9 |
| `web/Dockerfile` | The PureScript bundle and Playwright executor fold into the per-substrate container; on Apple Silicon Playwright runs from the host node install | Phase 5, Sprint 5.5 |
| `tools/python_quality.sh` | Replaced by `poetry run check-code` declared as a Poetry console script in each `pyproject.toml`; the repo carries no `.sh` files anywhere | Phase 4, Sprint 4.7 |
| `scripts/install-formatter.sh` | The Haskell style gate becomes a Cabal test target with `ormolu`/`hlint` declared as build-tool deps; no `.sh` shim is part of the supported path | Phase 1, Sprint 1.6; Phase 6, Sprint 6.1 |
| `python/poetry.lock` (committed) and per-substrate `python/<substrate>/poetry.lock` (committed) | Generated artifact; Poetry resolution is intentionally re-run per substrate at image build time on Linux and on demand on Apple Silicon | Phase 1, Sprint 1.7 |
| `tools/generated_proto/**/*.py`, `*.pyi`, and `__pycache__/*.pyc` (committed) | Generated artifact; regenerated by `protoc` during build | Phase 1, Sprint 1.7 |
| `web/spago.lock` (committed) | Generated artifact; PureScript dependency lock should not live in git | Phase 1, Sprint 1.7 |
| `tools/__pycache__/*.pyc` and `python/adapters/__pycache__/*.pyc` (committed) | Generated artifact; Python bytecode caches should not live in git | Phase 1, Sprint 1.7 |
| `web/src/Generated/Contracts.purs` (committed under `web/generated/` in older trees) | Generated artifact; rebuilt by `infernix internal generate-purs-contracts` on every web build | Phase 1, Sprint 1.7 |
| `python/adapters/` flat layout (without per-substrate subdirectories) | Replaced by `python/<substrate>/adapters/<engine>/` so the integration test column matches the substrate's engine selection from the README matrix | Phase 4, Sprint 4.7 |

## Pending Removal Details

The Pending Removal entries above land together with the substrate-container, Envoy Gateway
API, hygiene, and `poetry run check-code` migrations described in Phase 1 Sprint 1.7, Phase 3
Sprints 3.5 and 3.8, Phase 4 Sprints 4.7 and 4.9, and Phase 5 Sprint 5.5. As each demolition
lands, the corresponding row moves from this section to the Completed table below.
