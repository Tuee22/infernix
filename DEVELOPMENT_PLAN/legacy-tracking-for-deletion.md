...
No pending removals are currently tracked in this ledger. New cleanup work is added here when a
supported path still carries an explicitly obsolete or duplicate surface.
...
| `tools/edge_proxy.py` | the Python edge proxy has been removed; the supported path now runs `src/Infernix/Edge.hs` plus `src/Infernix/HttpProxy.hs` via the `infernix edge` entrypoint, and the integration suite exercises that process directly against mock upstreams | Phase 3, Sprint 3.5 |
| `tools/portal_surface.py` | the Python platform gateways have been removed; the supported path now runs `src/Infernix/Gateway.hs` plus `src/Infernix/HttpProxy.hs` via `infernix gateway harbor|minio|pulsar`, and the integration suite validates Harbor, MinIO, and Pulsar routing directly against mock upstreams | Phase 3, Sprint 3.8 |
...
| `chart/templates/deployment-web.yaml`, `chart/templates/service-web.yaml`, and the matching `infernix-web` compatibility deployment in `src/Infernix/Cluster.hs` | the routed `/` surface is served only by `infernix-demo`; the separate `web/Dockerfile` image remains Harbor-published for Playwright execution but is no longer deployed as its own cluster workload | Phase 5, Sprint 5.5 |
