...
|----------|-------------------------------|------------------------|
| `tools/runtime_worker.py`, `tools/runtime_backend.py`, `src/Infernix/Runtime.hs`, `test/integration/Spec.hs` engine fixture and command-prefix adapter path | the supported runtime now launches process-isolated engine workers through configured command prefixes, and automated validation injects `tools/engine_fixture.py` to prove the adapter contract without requiring every final third-party engine on the current host; this fixture-backed adapter surface remains a transitional compatibility path until supported-host validation lands for the real engines selected by the README matrix, and the routed publication plus integration surfaces currently assert `workerExecutionMode = process-isolated-engine-workers` and `workerAdapterMode = configured-engine-processes` | Phase 4, Sprint 4.2; Phase 4, Sprint 4.3; Phase 6, Sprint 6.2; Phase 6, Sprint 6.6 |
...
### Engine Fixture And Command-Prefix Adapter Placeholder

- `tools/runtime_worker.py` now shells into configured engine command prefixes and can inject
  `tools/engine_fixture.py` during automated validation so the process-isolated adapter contract is
  exercised without requiring every final engine binary or module on the current host
- `tools/runtime_backend.py` supervises those workers and reports the transitional contract through
  `workerExecutionMode = process-isolated-engine-workers`
...
- `test/integration/Spec.hs` currently treats the configured adapter plus fixture-command contract
  as the expected routed publication surface, so those assertions will need to narrow once the
