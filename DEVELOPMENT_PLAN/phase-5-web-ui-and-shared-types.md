...
- `cabal test infernix-unit` and `cabal run infernix -- lint {docs,files,chart,proto}` exit
  zero.
- Apple cohort validation is closed in [Waves A/A.2](cohort-validation-waves.md), and the CUDA Linux
  `linux-cpu` and `linux-gpu` gates pass.
...
- PureScript contract tests roundtrip each `InferenceError` constructor used by the backend, and
  decode a raw Haskell-wire `ModelMemoryLimitExceeded` server patch and server snapshot.
- Chat/view-model tests cover `ModelMemoryLimitExceeded` rendering from fields, a snapshot for a
  non-active context leaving the rendered pane intact, and a transiently stale active context id
  still rendering the stored conversation.
...
The sprint follows Sprint 4.28 in
[phase-4-inference-service-and-durable-runtime.md](phase-4-inference-service-and-durable-runtime.md),
which establishes the runtime-side typed `awaitModelBootstrapReady` evidence and the server ceiling
that this contract single-sources and the executor awaits.
...
- the model-bootstrap deadline is single-sourced through the shared Haskell browser contract in
  ...
  constructed: `modelBootstrapReadyServerCeilingSeconds` is canonical and
  `clientModelBootstrapDeadlineSeconds` derives the client deadline as that ceiling plus a `max 0`
  margin
- `Infernix.Runtime.Pulsar.modelBootstrapReadyWaitMaxSeconds` references the shared value, and
  `renderPursContractFooter` emits both constants into the generated browser contract
- the Playwright executor in `web/playwright/inference.spec.js` awaits the real
  `ModelBootstrapReadyEvent` as typed readiness evidence, bounded by
  `clientModelBootstrapDeadlineMs`, which mirrors the single-sourced client deadline and is `>=` the
  server ceiling
- the prior kubectl rollout proxy for readiness is removed from the executor path; the engine
  deployment is still scaled up, but readiness is proven by the real model-bootstrap flow, since the
  per-model inference result only arrives after the `ModelBootstrapReadyEvent`
