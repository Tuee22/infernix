...
| Playwright runtime | baked into `docker/linux-substrate.Dockerfile` (Node, the Playwright runtime, and the three browsers) and invoked from inside the outer container via `npm --prefix web exec -- playwright test` on Linux substrates; on Apple Silicon a host-native `npm --prefix web exec -- playwright test` lane is deferred and surfaces an explicit deferral diagnostic until that refactor lands. The retired dedicated `infernix-playwright:local` image and `docker/playwright.Dockerfile` were removed by Phase 3 Sprint 3.10 | open: Apple host-native `runRuntimeModeE2E` refactor (deferred to the Apple-validation pass) and the `linux-gpu` E2E rerun on the rebuilt launcher image |
| Compose launcher | `compose.yaml` | outer-container launcher for supported Linux workflows; defines exactly the `infernix` service with two bind mounts (`./.data` and `/var/run/docker.sock`) and no `environment:` block (the launcher image is selected by `${INFERNIX_COMPOSE_IMAGE:-infernix-linux-cpu:local}` substitution at compose-render time). The Phase 3 Sprint 3.10 `playwright` service removal landed on May 24, 2026 — Playwright now runs inside the same substrate image | none |
...
| Web runtime executor | PureScript bundle plus Playwright runtime (Node, the Playwright executor, and the three browsers) both baked into `docker/linux-substrate.Dockerfile` | substrate image runs cluster-resident as the demo app; routed E2E runs in-container on Linux substrates via `npm --prefix web exec -- playwright test ...` against the routed cluster on Docker's private `kind` network. The Apple host-native E2E refactor (host-side `npm exec` Playwright fed by the same typed fixture) is deferred and currently surfaces an explicit deferral diagnostic in `runRuntimeModeE2E`. Sprint 3.10 of Phase 3 retired the dedicated `infernix-playwright:local` image and `docker/playwright.Dockerfile` on May 24, 2026 | serves the browser bundle from the clustered demo app and runs routed E2E coverage from the same substrate image | test artifacts under `./.data/` |
