...
|----------|-----------|--------|-------|-------|
| Matrix registry -> staged substrate file | local staging boundary | banner-prefixed JSON under a legacy `.dhall` filename | `src/Infernix/DemoConfig.hs`, `src/Infernix/Models.hs` | Apple staging lives under `./.build/`; Linux outer-container staging lives under `./.build/outer-container/build/` on the host; the active substrate selects engine bindings consumed by `infernix service`, `infernix-demo`, and the integration suite |
