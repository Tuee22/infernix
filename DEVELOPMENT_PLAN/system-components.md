...
| Component | Technology | Deployment | Purpose | Durable State |
|-----------|------------|------------|---------|---------------|
| Kind and Helm lifecycle | Haskell control-plane orchestration in `cluster up` | host-native CLI or outer container | create or reuse the real Kind cluster, write the repo-local kubeconfig, reset StorageClasses, bootstrap declared Helm repositories, render the Helm chart, discover durable claims, reconcile PVs, build or load repo-owned images, and deploy the repo-owned chart | `./.data/runtime/cluster-state.state` plus `./.data/kind/...` |
| Bootstrap registry helper | repo-built Distribution registry image plus Kind registry-host rewrites | host-native CLI or outer container during `cluster up` | mirror the MinIO and Pulsar bootstrap images on `localhost:30001` before Harbor is ready, then let the final rollout switch those workloads to Harbor-published references | none |
...
|-----------|-------------|---------|
| Cluster reconcile | `infernix cluster up` | choose the edge port, create or reuse Kind, reset StorageClasses, reconcile PV directories and PV objects, generate `infernix-demo-<mode>.dhall`, publish the real ConfigMap and publication state, build or load repo-owned images, and deploy the Helm chart |
