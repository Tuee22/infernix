...
| Command registry | `optparse-applicative`-backed Haskell command registry | host or outer container | owns the supported command inventory, `--help` output, and the generated CLI-reference sections that docs lint enforces | none |
...
| Cluster-mounted substrate file | Helm deployment plus ConfigMap mount | `/opt/build/infernix-substrate.dhall` | cluster-resident consumers such as `infernix-demo` and all cluster `infernix service` workloads consume the cluster-role payload under the shared staged filename at `/opt/build/`; Apple host daemons read host-role config under `./.build/` |
