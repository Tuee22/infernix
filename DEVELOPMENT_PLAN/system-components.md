...
| Playwright runtime | baked into `docker/linux-substrate.Dockerfile` (Node, the Playwright runtime, and the three browsers) and invoked from inside the outer container via `npm --prefix web exec -- playwright test` on Linux substrates; on Apple Silicon the host-native lane now invokes `npm --prefix web exec -- playwright test` from the host CLI against the published localhost edge port using the same typed fixture. The rebuilt Linux launcher passed clean-env `linux-gpu` E2E on May 27, 2026 with `1 passed` | open: Apple host-native Playwright validation on Apple hardware |
