...
- the daemon-role contract is code-side implemented around substrate-neutral engine pools: on every
  ...
  pool subscription, and production `demo_ui = false` route/publication assertions, while Wave J
  still owns single-host logical shared-pool backlog/backpressure and Linux placement evidence.
