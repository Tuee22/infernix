...
|-----------|------------|------------|---------|---------------|
| Apple host control plane | `./.build/infernix` plus direct `cabal` materialization and host prerequisite detection | host-native | canonical current operator surface on Apple, including repo-owned Python prerequisite installation when manifests require it and no repo-owned script-wrapper layer | `./.build/` plus `./.data/` |
...
| Service runtime host | Python HTTP server launched by `infernix service` through the Haskell CLI | host process or service pod | serve the API, routed publication metadata, and large-object references using the active generated demo catalog, repoint `/api` through the Apple host bridge when the daemon runs host-native, use cluster-local MinIO or Pulsar endpoints from the service pod while host-native placement uses the edge-routed `/minio/s3`, `/pulsar/admin`, and `/pulsar/ws` bridges, launch process-isolated engine-worker runners through configured command prefixes including forwarded `INFERNIX_ENGINE_COMMAND_*` overrides, and materialize durable runtime artifact bundles plus engine-specific source-artifact manifests into the runtime bucket without exposing implicit filesystem service fallback | runtime cache under `./.data/runtime/` |
