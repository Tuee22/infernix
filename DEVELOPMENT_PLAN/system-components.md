...
| Demo conversation Pulsar topics | Pulsar topic family `persistent://infernix/demo/demo.conversation.<userId>.<contextId>` | Pulsar broker, demo-gated | per-context append-only conversation log; single-partition, broker-assigned `MessageId` is the canonical sequence; producer-side dedup enabled; May 28, 2026 Linux GPU integration validates real publish + Reader decode, duplicate frontend publish collapse, and completed result writeback from a non-chaos dispatcher/result-bridge prompt roundtrip; the 2026-06-02 LinuxCpu integration code asserts exactly-one request/batch/result/conversation-result counts through frontend, coordinator, engine, and node-drain chaos paths; absent when `demo_ui = false` | Pulsar BookKeeper state plus tiered storage offload to MinIO |
...
| Model bootstrap topic | Pulsar topic `persistent://infernix/system/model.bootstrap.request` plus the `model.bootstrap.ready.<modelId>` family | Pulsar broker, always-on | exactly-once model population workflow: engine sees an uncached model → publishes a bootstrap request with producer dedup keyed by `modelId` → coordinator's Failover subscription downloads from the upstream URL in the substrate `.dhall` → uploads to `infernix-models/<modelId>/` → writes `.ready` sentinel last → publishes ready event keyed by `modelId`; May 28, 2026 Linux GPU integration validates real ready-topic publish + Reader decode with key assertion; the 2026-06-02 LinuxCpu integration code publishes duplicate bootstrap requests around coordinator replacement and asserts exactly one ready event; the coordinator is the only daemon with outbound-internet egress to upstream model hosts | Pulsar BookKeeper state |
...
| `/objects/<objectRef>` | retired | legacy demo object-store fixture access; retired by Phase 7 Sprint 7.7 | absent |
