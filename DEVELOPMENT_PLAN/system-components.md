...
- the daemon-role contract is code-side implemented around substrate-neutral engine pools: on every
  ...
  pool subscription, and production `demo_ui = false` route/publication assertions. Current source
  also adds a compile-validated single-host logical `Shared` backlog harness, while Wave J still
  owns real Apple execution of that harness and Linux placement evidence.
