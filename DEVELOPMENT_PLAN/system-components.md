...
- the daemon-role contract is code-side implemented around substrate-neutral engine pools: on every
  ...
  through derived pool/model topics; Wave J still owns real-cluster backpressure and pinned-route
  evidence. Existing workers use typed Python or native adapter harnesses, fetch model weights lazily
  from `infernix-models`, and publish the typed per-family result surface; hardware proof for real
  output remains in cohort gates. Unsupported adapter ids fail fast instead of returning a generic
  success payload
