...
- Linux outer-container lifecycle runs forward the host repo root so generated Kind or `nvkind`
  ...
  `./.build/kind/registry/` directories directly into node containers instead of replaying
  retained state with `docker cp`
...
| Linux outer-container control plane | `docker compose --project-name <lane> --file compose.yaml ... run --rm infernix infernix ...` | Linux container | only supported Linux CLI surface for `linux-cpu` and `linux-gpu`; the GPU lane adds `--file compose.linux-gpu.yaml` to select the CUDA snapshot, forwards the Docker socket, and bind-mounts only `./.data/` while the build root, source snapshot, and chart archives stay in the image overlay; uses a launcher-local scratch kubeconfig under system temp for Kind or `nvkind` create or delete before publishing the durable repo-local kubeconfig | `./.data/`, `./.data/runtime/infernix.kubeconfig` |
...
| Substrate configuration | staged typed Dhall record at `infernix-substrate.dhall`, decoded in-process by the `dhall` Haskell library | host or outer container | primary source of truth for active substrate, generated catalog content, daemon role, inference placement, Pulsar request/result/batch topics, active engine dispatch, routed Apple bridge behavior, and test scope once the file has been staged; materialization writes the staged file atomically so concurrent readers never observe a partial payload | `./.build/infernix-substrate.dhall` on Apple carries host-role metadata; `./.build/outer-container/build/infernix-substrate.dhall` on the Linux outer-container path carries cluster-role metadata; cluster pods mount the cluster-role payload at `/opt/build/infernix-substrate.dhall` |
...
| Playwright runtime | baked into `docker/linux-substrate.Dockerfile` (Node, the Playwright runtime, and the three browsers) and invoked from inside the outer container via `npm --prefix web exec -- playwright test` on Linux substrates; on Apple Silicon a host-native `npm --prefix web exec -- playwright test` lane is deferred and surfaces an explicit deferral diagnostic until that refactor lands. The rebuilt Linux launcher passed clean-env `linux-gpu` E2E on May 27, 2026 with `1 passed` | open: Apple host-native `runRuntimeModeE2E` refactor, deferred to the Apple-validation pass |
| Compose launcher | `compose.yaml`, `compose.linux-gpu.yaml` | outer-container launcher for supported Linux workflows; the base file defines exactly the `infernix` service with two bind mounts (`./.data` and `/var/run/docker.sock`) and no `environment:` block, while the GPU override selects `infernix-linux-gpu:local`. The Phase 3 Sprint 3.10 `playwright` service removal landed on May 24, 2026 — Playwright now runs inside the same substrate image | none |
...
| Service runtime daemons | `infernix service` plus `src/Infernix/Runtime/{Cache,Worker,Pulsar}.hs`, `src/Infernix/Dispatch/SingleFlight.hs`, `src/Infernix/Bridge/Result.hs`, `src/Infernix/Bootstrap/Models.hs` | cluster pods on every substrate plus host process for Apple inference execution | the supported three-role daemon model (see [../documents/architecture/daemon_topology.md](../documents/architecture/daemon_topology.md)) splits Pulsar coordination from engine execution. The coordinator role (`infernix-coordinator` Deployment, stateless, replicas ≥ 2, soft anti-affinity, **no PVC**) runs the single-flight dispatcher, the result-bridge, and the model-bootstrap worker under Pulsar named `Failover` subscriptions; the engine role (`infernix-engine` Deployment on Linux substrates with **required** one-per-node anti-affinity, **no PVC**, single `emptyDir` `model-cache` mount with hard `sizeLimit`; on-host daemon on Apple silicon with `flock(2)` singleton on `engine.lock`) consumes `inference.batch.<mode>`, pulls weights from the `infernix-models` MinIO bucket into ephemeral pod storage via the shared adapter helper, runs the adapter, owns the node's KV cache (in-memory), and publishes `inference.result.<mode>`. Worker execution uses typed Python or native adapter harnesses, with unsupported adapter ids failing fast instead of returning a generic success payload. When Pulsar endpoints are intentionally absent in unit-level harnesses, the same daemon code can use the repo-local topic spool under `./.data/runtime/pulsar/`; that harness path is not routed cluster evidence. Phase 7 Sprint 7.7 (May 23, 2026) landed the daemon split: `chart/templates/deployment-service.yaml` is gone, `clusterServiceEnabled` returns `False` on every substrate, and `finalPhaseDeployments` waits on `deployment/infernix-{coordinator,engine}` plus the demo-gated `infernix-demo`. The coordinator's runtime loops (`runResultBridgeLoop` for the per-context result writeback, `runModelBootstrapLoop` for the lazy `infernix-models` upload) are forked by `runProductionDaemon` when `daemonRole == Coordinator`; the engine config has `daemonConfigHostBatchTopic = Nothing` so it executes the worker inline and publishes results. | none on cluster daemons (Pulsar cursors are broker-side; KV cache is in-memory); the engine pod's `emptyDir` `model-cache` is ephemeral per-pod storage capped by `sizeLimit` and rebuilds from the `infernix-models` MinIO bucket on every pod start |
| Demo UI host | `infernix-demo` deployment | cluster pod | serves `/`, `/api`, `/api/publication`, `/api/demo-config`, `/api/models`, `/api/cache`, `/api/objects/{upload,download}`, and `/ws` when demo is enabled; routed manual inference closes through the Phase 7 durable-context Chat surface, where the `infernix-demo` Pulsar-fronted dispatcher publishes inference work and (on Apple) hands batches off to host inference daemons; direct `infernix-demo` execution intentionally exposes only the demo-owned HTTP surface outside the intended HTTPRoute mapping. The retired `/objects/:objectRef` route is removed in Phase 7 Sprint 7.7 | none |
| Web runtime executor | PureScript bundle plus Playwright runtime (Node, the Playwright executor, and the three browsers) both baked into `docker/linux-substrate.Dockerfile` | substrate image runs cluster-resident as the demo app; routed E2E runs in-container on Linux substrates via `npm --prefix web exec -- playwright test ...` against the routed cluster on Docker's private `kind` network. The Apple host-native E2E refactor (host-side `npm exec` Playwright fed by the same typed fixture) is deferred and currently surfaces an explicit deferral diagnostic in `runRuntimeModeE2E` | serves the browser bundle from the clustered demo app and runs routed E2E coverage from the same substrate image | test artifacts under `./.data/` |
...
| Inference batch topics (Planned, Phase 7 Sprint 7.7 generalizes to every substrate) | Pulsar topic family `persistent://infernix/demo/inference.batch.<mode>` | Pulsar broker | coordinator role publishes pre-batched inference work to this topic with producer dedup keyed by `batchId`; engine role consumes from it; today this exists only for Apple as `inference.batch.apple-silicon.host` and is generalized to `linux-cpu` and `linux-gpu` in Sprint 7.7 | Pulsar BookKeeper state |
| Platform model bucket (Planned, Phase 7 Sprint 7.7) | MinIO bucket `infernix-models` | Kind cluster, always-on (not demo-gated) | platform-owned model weights, tokenizers, and configs; populated lazily on first use by the coordinator's bootstrap Failover subscription; per-model `.ready` sentinel object written last marks an atomic publish; engine pods pull from here into their `emptyDir` model cache | MinIO durable state under `./.data/kind/<runtime-mode>/...` |
| Model bootstrap topic (Planned, Phase 7 Sprint 7.7) | Pulsar topic `persistent://infernix/system/model.bootstrap.request` plus the `model.bootstrap.ready.<modelId>` family | Pulsar broker, always-on | exactly-once model population workflow: engine sees an uncached model → publishes a bootstrap request with producer dedup keyed by `modelId` → coordinator's Failover subscription downloads from the upstream URL in the substrate `.dhall` → uploads to `infernix-models/<modelId>/` → writes `.ready` sentinel last → publishes ready event; the coordinator is the only daemon with outbound-internet egress to upstream model hosts | Pulsar BookKeeper state |
...
| Cluster status | `infernix cluster status` | report cluster presence, the active substrate through its current `runtimeMode` line, publication state, build or data roots, route inventory, and the active in-progress lifecycle action, phase, detail, and heartbeat fields without mutating Kubernetes resources, publication state, or authoritative repo-local state; on Linux outer-container paths it may idempotently attach the fresh launcher container to Docker's private `kind` network for observation |
...
| `/objects/<objectRef>` | HTTPRoute -> `infernix-demo` Service | demo object-store fixture access; **retired by Phase 7 Sprint 7.7** — browsers fetch generated artifacts via presigned GET URLs minted at `/api/objects` from MinIO directly, never through the demo backend | currently registered; absent when `demo_ui` is false; removed entirely after Sprint 7.7 |
...
|----------|-----------|--------|-------|-------|
| Matrix registry -> staged substrate file | local staging boundary | typed Dhall record at `infernix-substrate.dhall`, schema at `dhall/InfernixSubstrate.dhall` | `src/Infernix/DemoConfig.hs`, `src/Infernix/Models.hs` | Apple staging lives under `./.build/`; Linux outer-container staging lives under `./.build/outer-container/build/` on the host; the active substrate selects engine bindings and daemon roles consumed by cluster daemons, Apple host daemons, `infernix-demo`, and the integration suite |
...
| Coordinator -> engine | internal production path on every substrate | protobuf batches over the dedicated `inference.batch.<mode>` Pulsar topic family | `src/Infernix/Runtime/Pulsar.hs` plus coordinator-role and engine-role `.dhall` config | the coordinator role publishes pre-batched inference work to `inference.batch.<mode>` with producer dedup; the engine role consumes from it. On Apple the engine role is the on-host daemon and the topic remains `inference.batch.apple-silicon`; on Linux substrates both daemons run as separate in-cluster Deployments after Sprint 7.7 of Phase 7. Pulsar-owned topics, exclusive subscriptions, and acknowledgement handling keep batch ownership unambiguous |
...
| Helm dependency archive cache | `cluster up`, `test integration`, `test all`, and any supported chart-reconcile path that calls `ensureHelmDependencies` | image-local `chart/charts/` on the Linux outer-container path; `chart/charts/` in the Apple host worktree | cached top-level Helm dependency archives for Harbor, PostgreSQL, Pulsar, MinIO, and Envoy Gateway |
...
| Durable runtime artifact bundles | service runtime and cache materialization (current; **retired by Phase 7 Sprint 7.7** — bundles disappear once weights come from the `infernix-models` MinIO bucket and engine pods cache to `emptyDir`) | `./.data/object-store/artifacts/<substrate>/<model-id>/bundle.json` | durable worker input metadata; tracked in `legacy-tracking-for-deletion.md` |
| Durable source-artifact manifests | service runtime and cache materialization (current; **retired by Phase 7 Sprint 7.7** alongside artifact bundles) | `./.data/object-store/source-artifacts/<substrate>/<model-id>/source.json` | authoritative artifact-selection metadata; tracked in `legacy-tracking-for-deletion.md` |
...
| Host-side cache durability manifests | service runtime and tests (current; **retired by Phase 7 Sprint 7.7** — cache rebuildability comes from the Pulsar conversation log via `prefixHash`, no host-side manifest needed) | `./.data/object-store/manifests/<substrate>/<model-id>/default.pb` | rebuild source for derived cache; tracked in `legacy-tracking-for-deletion.md` |
