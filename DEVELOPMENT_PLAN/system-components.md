...
| Apple tart Metal-engine build lane | `tart` macOS VM driven by `src/Infernix/Engines/AppleSilicon.hs` plus `infernix internal materialize-metal-engines`; `tart` reconciled by Homebrew (`brew install tart`) and recorded as `hostTart` in `dhall/InfernixHost.dhall` | host-native ephemeral macOS guest | builds the Metal and Core ML native engine artifacts that would otherwise require Xcode (`llama.cpp` and `whisper.cpp` Metal, Core ML basic-pitch, Apple Stable Diffusion Core ML, the Omnizart Core ML export) inside a headless tart VM, copies them to the host engine roots, then destroys the VM; the host runs them against Metal, keeping the host free of Xcode. `tart` is native arm64 macOS virtualization, distinct from Docker and Colima, and creates or switches no Docker context | tart-built artifacts under `./.data/engines/<adapterId>/` |
