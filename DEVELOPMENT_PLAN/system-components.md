...
|-----------|------------|------------|---------|---------------|
| Apple host control plane | `./.build/infernix` plus direct `cabal` materialization | host-native | canonical operator surface on Apple, with no Python prerequisite for the operator workflow; Poetry plus `./.venv/` materialize only when an engine-adapter test is exercised explicitly | `./.build/` plus `./.data/` |
...
| Frontend contract generator | `infernix internal generate-purs-contracts` plus `spago bundle-app` | host or outer container during web build | emit the build-generated PureScript contract modules from Haskell ADTs in `src/Infernix/Demo/Api.hs` via `purescript-bridge` into `web/src/Generated/`, then bundle the demo UI through `spago bundle-app` into `web/dist/` | `web/src/Generated/` plus `web/dist/` |
...
| Harbor gateway workload | Haskell gateway in `infernix-lib` (`src/Infernix/Gateway/Harbor.hs`); deployed as cluster workload `infernix-harbor-gateway` via `chart/templates/workloads-platform-portals.yaml` using the same OCI image with entrypoint `infernix gateway harbor` | Kind cluster | expose the routed Harbor portal and API surface by proxying the chart-managed Harbor service through the shared edge | none |
| MinIO gateway workload | Haskell gateway in `infernix-lib` (`src/Infernix/Gateway/Minio.hs`); deployed as cluster workload `infernix-minio-gateway` via `chart/templates/workloads-platform-portals.yaml` using the same OCI image with entrypoint `infernix gateway minio` | Kind cluster | expose the routed MinIO console and S3 API by proxying the chart-managed MinIO service through the shared edge | none |
| Pulsar gateway workload | Haskell gateway in `infernix-lib` (`src/Infernix/Gateway/Pulsar.hs`); deployed as cluster workload `infernix-pulsar-gateway` via `chart/templates/workloads-platform-portals.yaml` using the same OCI image with entrypoint `infernix gateway pulsar` | Kind cluster | expose the routed Pulsar admin and WebSocket surfaces by proxying the chart-managed Pulsar service through the shared edge | none |
...
| Service runtime host | Haskell daemon `infernix service` (production); Pulsar consumer/dispatcher built on `src/Infernix/Runtime/{Pulsar,Worker,Cache}.hs`; binds no HTTP listener | host process or `infernix-service` cluster pod | subscribe to request topics named in the active `.dhall`, dispatch each request through the Haskell worker, publish results to result topics named in the same config, materialize durable runtime artifact bundles plus engine-specific source-artifact manifests into the runtime bucket, and fork Python adapters from `python/adapters/<engine>/` only when the bound engine is Python-native; cluster-resident placement uses cluster-local MinIO or Pulsar endpoints, while host-native placement uses the edge-routed `/minio/s3`, `/pulsar/admin`, and `/pulsar/ws` bridges | runtime cache under `./.data/runtime/` |
...
| Web runtime host | static PureScript bundle from `web/dist/` (produced by `spago bundle-app`) served by the web image built from `web/Dockerfile` | web pod on the supported cluster path | host the demo UI assets behind `infernix-demo` | none |
| Engine adapter container set | Python adapters under `python/adapters/<engine>/` from one repo-root `python/pyproject.toml` (Poetry-managed); each adapter container build runs `tools/python_quality.sh` (mypy strict, black check, ruff strict) and fails on any check failure; Poetry installs system-wide inside the container, with no in-container `.venv` | per-engine cluster pod (or sidecar to the worker) | bind PyTorch, JAX, and other Python-native engines to the Haskell worker over typed protobuf-over-stdio (or unix socket) | none |
| Python quality gate | `tools/python_quality.sh` invoking mypy strict, black check, and ruff strict against `python/` | inside every adapter container build; on host via `infernix test lint` against `./.venv/` | enforce the strict typing and formatting contract on all repo-owned Python | none |
| Pulsar inference subscription | Haskell consumer in `src/Infernix/Runtime/Pulsar.hs` invoked by `infernix service` (production) | `infernix-service` cluster pod (production) or host-native daemon (Apple dev) | subscribe to request topics named in the active `.dhall`, dispatch through the worker, publish results to configured result topics; this is the production inference surface and binds no HTTP port | none |
...
| Service runtime | `infernix service` | resolve the active generated-catalog source and run the Haskell Pulsar consumer/dispatcher used directly on Apple or inside the `infernix-service` pod on the cluster path; binds no HTTP port |
...
| Frontend contract generation | `infernix internal generate-purs-contracts` | generate PureScript contract modules from Haskell ADTs in `src/Infernix/Demo/Api.hs` via `purescript-bridge` into `web/src/Generated/` |
| Unit validation | `infernix test unit` | validate Haskell runtime behavior plus `purescript-spec` suites covering generated contracts, view modules, and catalog rendering logic |
| Integration validation | `infernix test integration` | exercise every generated catalog entry for the active runtime mode and verify generated demo-config publication, routed publication metadata, the real in-cluster ConfigMap, process-isolated engine-worker execution, durable runtime artifact plus engine-specific source-artifact persistence and selected-artifact inventory, the engine-specific default runner path plus any adapter-specific overrides, operator-managed PostgreSQL readiness or failover or deterministic PVC rebinding, and real `linux-cuda` GPU visibility on supported hosts |
| Routed E2E validation | `infernix test e2e` | exercise every generated catalog entry through the real routed cluster edge using Playwright launched from the same web image that serves the UI, including browser UI interaction, object-reference result rendering, and the current process-isolated engine-worker runner contract |
...
|--------------|-------------------|-------------------------------|-------------------|
| Apple Silicon / Metal | `apple-silicon` | active host-native and Kind-backed catalog lane; request execution uses process-isolated engine-worker runners, durable runtime bundles, engine-specific source-artifact manifests, authoritative source-artifact selection, engine-adapter metadata, and the default engine-aware runner plus adapter-specific command overrides in automated validation | generated catalog includes every README matrix row whose Apple column names a supported engine |
| Ubuntu 24.04 / CPU | `linux-cpu` | active Kind-backed catalog lane on the host-native and outer-container control planes; request execution uses process-isolated engine-worker runners, durable runtime bundles, engine-specific source-artifact manifests, authoritative source-artifact selection, engine-adapter metadata, and the default engine-aware runner plus adapter-specific command overrides in automated validation | generated catalog includes every README matrix row whose Linux CPU column names a supported engine |
| Ubuntu 24.04 / NVIDIA CUDA Container | `linux-cuda` | active Kind-backed catalog lane with supported-host NVIDIA preflight checks, `nvkind` cluster creation, Helm-installed NVIDIA device plugin, `RuntimeClass/nvidia`, GPU-requesting repo-owned workloads, and the same process-isolated engine-worker or authoritative-source-artifact contract plus engine-adapter metadata used on the other runtime modes | generated catalog includes every README matrix row whose Linux CUDA column names a supported engine and marks GPU-bound lanes |
...
| Browser <-> demo API | external (demo-only) | JSON over HTTP | Haskell-owned types in `src/Infernix/Demo/Api.hs`; PureScript modules generated by `purescript-bridge` into `web/src/Generated/` | demo UI only; production deployments do not bind this surface |
...
| Generated frontend contract staging | `infernix internal generate-purs-contracts` | `web/src/Generated/` | PureScript modules emitted from Haskell ADTs in `src/Infernix/Demo/Api.hs` via `purescript-bridge`; consumed by `spago build` |
| Generated frontend dist | `spago bundle-app` invoked from `web/Dockerfile` | `web/dist/` | ignored static build output served by the web runtime image and rendered by `infernix-demo` |
