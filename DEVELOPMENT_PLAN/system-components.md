...
- the daemon-role contract is code-side implemented around substrate-neutral engine pools: on every
  ...
  demo-off publication. Wave J still owns Linux GPU/CUDA cohort proof.
  ...
  `infernix-models`, and publish the typed per-family result surface; hardware proof for real output
  remains in cohort gates. Unsupported adapter ids fail fast instead of returning a generic success
  payload
...
- legacy validation proof points are inventoried in
  ...
  daemon, and current Wave J evidence adds Linux CPU engine-pool placement/backpressure validation
  on the native arm64 Docker lane
...
| Apple headless Metal/Core ML materialization lane | fixed host Metal runtime bridge plus typed engine-artifact manifests; replacement for the legacy `tart` / `hostTart` / `AppleTart` implementation, with the retained `materialize-metal-engines` command as the manifest surface | host-native Apple process | materializes Apple Metal/Core ML and native runner roots without Tart, user keychain state, host Xcode UI flows, or request-time toolchain installation; Metal source compilation uses the OS Metal runtime compiler, Core ML smoke probes link Foundation/CoreML, and current native runner roots are deterministic validation wrappers pending Wave I real payload replacement | engine artifacts under `./.data/engines/<adapterId>/`; optional immutable payloads in `infernix-engine-artifacts` |
| Linux native engine materialization lane | `src/Infernix/Engines/LinuxNative.hs` plus `infernix internal materialize-linux-native-engines` | Linux substrate image build | writes typed engine-artifact manifests and smoke-validated native runner roots under `/opt/infernix/engines/<adapterId>/` for the runnable Linux native adapter ids; the current payloads are runner-contract payloads that parse native argv, fail with exit 75 until `<model-cache-root>/<model-id>/.ready` exists, emit optional worker-upload artifact markers, and remain pending Wave I external-engine replacement | image-owned `/opt/infernix/engines/<adapterId>/`; optional immutable payloads in `infernix-engine-artifacts` |
...
| Per-engine framework venvs (Phase 4 Sprint 4.16) | `python/engines/<engine>/pyproject.toml` for `transformers`, `vllm`, `pytorch`, `tensorflow`, `jax`, `diffusers` | isolated in-project venvs that path-depend on the shared `infernix-adapters` package and install their mutually-conflicting framework wheels in an optional substrate group (`--with cuda` on linux-gpu, cu128 torch for Blackwell; `--with apple-silicon` on Apple host-native `transformers`, `pytorch`, and `diffusers` engines; `--with linux-cpu` for Linux CPU `transformers` and `pytorch` validation engines); `src/Infernix/Runtime/Worker.hs` runs the per-engine venv when present and falls back to the fail-fast shared path when absent; Linux CPU and Linux GPU builds bake their applicable venvs as resilient layers. Linux CPU Transformers returns a deterministic tokenizer/config smoke response and Linux CPU Bark emits a deterministic WAV validation artifact; full real-output fidelity remains Wave I/CUDA or native-payload work. Basic Pitch TensorFlow, Omnizart, and MT3 are named cohort residuals | `python/engines/<engine>/.venv/` (gitignored build artifact) |
...
| Component | Technology | Deployment | Purpose | Durable state |
|-----------|------------|------------|---------|---------------|
| Kind and Helm lifecycle | Haskell control-plane orchestration in `cluster up` | host-native Apple CLI or Linux outer container | create or reuse Kind, scrub non-retained operator-managed Patroni PostgreSQL claim roots before recreating claim directories and after retained-state sync, reset StorageClasses, reconcile PVs, deploy Harbor first, publish the cluster-role substrate payload, perform Harbor-first image preparation, deploy the final chart including the role-specific coordinator and engine daemon Deployments, expose in-progress lifecycle phase, detail, and heartbeat state for status observers, retry once with a targeted Pulsar claim-root reset when retained ZooKeeper state is self-inconsistent, reinitialize stopped retained Harbor PostgreSQL replicas from the current Patroni leader when timeline drift leaves replicas unready after promotion, and recycle unready Harbor PostgreSQL startup pods with non-waiting deletes so StatefulSet name reuse cannot block lifecycle progress | `./.data/runtime/cluster-state.state`, `./.data/kind/<runtime-mode>/...` |
...
| Service runtime daemons | `infernix service` plus `src/Infernix/Runtime/{Daemon,Cache,KVCache,Worker,Pulsar}.hs`, `src/Infernix/Dispatch/SingleFlight.hs`, `src/Infernix/Bridge/Result.hs`, `src/Infernix/Bootstrap/Models.hs` | cluster pods on every substrate plus host processes for Apple inference execution | the supported target is the three-role daemon model plus substrate-neutral engine pools (see [../documents/architecture/daemon_topology.md](../documents/architecture/daemon_topology.md) and [../documents/architecture/engine_pool_routing.md](../documents/architecture/engine_pool_routing.md)). The coordinator role (`infernix-coordinator`, stateless, replicas >= 2, no PVC) owns request-topic fan-in, batching, model-to-pool routing, result-bridge writeback, and lazy model bootstrap. Engine members consume derived pool/model topics, pull weights from `infernix-models` through the shared adapter helper, run the selected adapter, own process-local KV cache, and publish `inference.result.<mode>`. Normal pools use Pulsar `Shared` and broker-native backpressure; pinned member routes use derived per-member topics with `Exclusive`. Linux members are Kubernetes workloads; Apple members are host daemons selected by stable host id. The current worktree still carries the legacy-compatible `engineDaemons` projection so older chart and daemon selection surfaces can be retired deliberately; that cleanup is tracked in Phase 4 / Phase 7 and the deletion ledger. Production-shaped `demo_ui = false` keeps the coordinator and engine pools while omitting demo-only workloads and routes. | none on cluster daemons (Pulsar cursors are broker-side; KV cache is in-memory); Linux engine pod `emptyDir` model caches and Apple host model caches are derived and rebuildable |
...
| Inference batch topics | Derived pool/model topic family `persistent://infernix/demo/inference.batch.<mode>.pool.<poolId>.model.<modelId>` plus pinned-member topic family `persistent://infernix/demo/inference.batch.<mode>.member.<memberId>.model.<modelId>` | Pulsar broker | the coordinator publishes pre-batched inference work only to topics derived from the validated engine-pool graph. Normal pool topics use `Shared` so broker permits and receiver backlog distribute work; pinned member topics use `Exclusive`. The old `inference.batch.<mode>`, `inference.batch.<mode>.<engine>`, and Apple `.host` topic helpers remain compatibility surfaces until the deletion-ledger cleanup removes them. | Pulsar BookKeeper state |
| Platform model bucket | MinIO bucket `infernix-models` | Kind cluster, always-on (not demo-gated) | platform-owned model weights, tokenizers, and configs; populated lazily on first use by the coordinator's bootstrap Failover subscription; per-model `.ready` sentinel object written last marks an atomic publish; Linux engine pods and Apple host engine members pull from here into derived model caches | MinIO durable state under `./.data/kind/<runtime-mode>/...` |
| Model bootstrap topic | Pulsar topic `persistent://infernix/system/model.bootstrap.request` plus the `model.bootstrap.ready.<modelId>` family | Pulsar broker, always-on | exactly-once model population workflow: engine sees an uncached model → publishes a bootstrap request with message key `modelId` and attempt-scoped producer dedup key `modelId@requestedAt` → coordinator's Failover subscription downloads from the upstream URL in the substrate `.dhall` → uploads to `infernix-models/<modelId>/` → writes `.ready` sentinel last → publishes ready event keyed by `modelId`; exact request replays collapse while later attempts can enqueue if readiness never appears, and the MinIO `.ready` guard prevents duplicate effective population; the integration suite validates real ready-topic publish + Reader decode with key assertion, and publishes duplicate bootstrap requests around coordinator replacement asserting exactly one ready event; the coordinator is the only daemon with outbound-internet egress to upstream model hosts | Pulsar BookKeeper state |
