...
> **Target contract.** The ledger and the declared-ceiling invariant below are the required end
> state. Sprint 0.19 authors this doctrine; [Sprint 1.21](../../DEVELOPMENT_PLAN/phase-1-repository-and-control-plane-foundation.md)
> owns the build-memory kernel and the generated ceiling, and
> [Sprint 6.46](../../DEVELOPMENT_PLAN/phase-6-validation-e2e-and-ha-hardening.md) owns the spawn
> boundary and its lint. The Apple lane is measured by its own cohort wave, because Darwin has
> neither cgroups nor an enforced `RLIMIT_AS`.
...
The doctrine is authored and registered; the implementation is scheduled and not yet landed.

- **Landed**: this document, its registration in the governed suite, and the non-negotiable rule
  mirrored into the two entry documents. An untracked stopgap ceiling protects the development host
  in the interim and is superseded by the generated form.
- **Sprint 1.21 (open)**: the bounded reservation on the built executable, the calibrated committed
  ceiling, the build-memory kernel with its hidden-constructor budget and concurrency, the
  generated per-machine ceiling, and physical-memory facts in the host manifest.
- **Sprint 6.46 (open)**: the closed toolchain invocation vocabulary, the unexported raw spawn, the
  `unboundedToolchainSpawnViolations` lint, the negative-compilation fixtures, the per-lane
  mechanism resolver, and the child victim rank.
- **Deferred, and named so it is not mistaken for closed**: the partition does not yet carry a
  build term, so the sum of declared claims is still not checked against physical memory; the sum
  of cluster pod limits is still not checked against node allocatable; and the nested builds that
  produce the setup helper and the formatter tools are still uncapped.
- The predecessor claims this document narrows are recorded in
