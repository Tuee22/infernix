...
- `infernix test lint` validates repository hygiene, required chart or Kind or `.proto` assets, the
  ...
  gate via `poetry --directory python run check-code` when adapters are present
...
- `infernix test integration` validates cluster lifecycle across the selected runtime-mode set,
  ...
  simulation, `cluster status`, demo-ui disablement on the `linux-cpu` lane, and edge-port
  rediscovery on the host-native `linux-cpu` lane
...
- `infernix test integration` serializes the active runtime mode into the generated demo config and
  publication state, then smoke-tests the routed demo API, auxiliary routed prefixes, one routed
  inference request, cache mutation endpoints, and the daemon request or result loop for each
