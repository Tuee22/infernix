...
- the supported validation contract exercises the process-isolated engine-worker adapter layer,
  the repo-owned default engine probe plus any adapter-specific command overrides, durable runtime
  bundles, direct-upstream source-artifact manifests, and the `linux-cuda` Kind path when the host
  satisfies the documented NVIDIA preflight contract
