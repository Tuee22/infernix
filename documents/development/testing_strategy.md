...
- `infernix test unit` validates matrix rendering, deterministic runtime behavior, protobuf schema
  ...
  Python-native bindings, and the current web contract or view behavior via `spago test` running
  `web/test/Main.purs`
...
- `infernix test integration` validates lifecycle, generated demo-config publication,
  ...
  Haskell-worker execution dispatching across the process-isolated Python adapter boundary for
  Python-native bindings, service-path cache lifecycle, the current production no-HTTP daemon
  contract, MinIO durability, operator-managed PostgreSQL readiness or primary failover or PVC
  rebinding, HA recovery, edge-port rediscovery, and real `linux-cuda` device visibility on
  supported hosts
- `infernix test e2e` validates the demo-only browser-facing surface (when the demo flag is on)
  ...
  against the real cluster edge while launching Playwright from the same web image that packages
  the built demo bundle
...
- the host-native and outer-container control-plane paths both delegate `infernix test e2e`
  browser execution to the same web image that packages the built demo bundle rather than the
  control-plane image, and the host-native final-substrate lane reuses the Harbor-published web
  runtime image for that browser execution path across `apple-silicon`, `linux-cpu`, and
  `linux-cuda`
