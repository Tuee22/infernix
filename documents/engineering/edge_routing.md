...
- the chart-owned route contract lives in `GatewayClass/infernix-gateway`,
  `Gateway/infernix-edge`, and the HTTPRoute inventory under `chart/templates/httproutes/`
...
- `/harbor`, `/minio/console`, `/minio/s3`, `/pulsar/admin`, and `/pulsar/ws` each use
  `URLRewrite` to strip the public prefix before forwarding to the chart-managed backend service
