...
- engine software artifacts are a separate class from model weights. The materialization
  ...
  Linux image-build helper writes runner-contract payloads at those roots; Wave I replaces them
  with real native payloads before real-output sign-off.
...
- the runtime worker invokes the engine for the selected binding — the Python adapter
  ...
  Linux native runner-contract payloads prove the native worker argument, model-cache readiness
  failure, local artifact marker, and result-shape contract; Wave I replaces them with real
  external engine payloads.
...
- per-engine software manifests record adapter id, engine name, substrate, architecture,
  ...
  `llm-qwen25-safetensors`); the full Apple e2e/all gate remains in Wave I. Linux
  runner-contract validation exists today and Wave I replaces those payloads with real native
  engines.
