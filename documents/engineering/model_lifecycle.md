...
- engine software artifacts are a separate class from model weights. The materialization
  ...
  Linux image-build helper writes smoke wrappers at those roots; Wave I replaces them with real
  native payloads before real-output sign-off.
...
- the runtime worker invokes the engine for the selected binding — the Python adapter
  ...
  Linux native smoke wrappers fail real inference until Wave I replaces them with real payloads.
