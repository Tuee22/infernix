...
- the runtime worker invokes the real engine for the selected binding — the Python adapter
  ...
  fallback for native-process-runner bindings — and runs real per-family inference, never a
  synthetic echo. Each family resolves to its own real lifecycle: the LLM and speech families
