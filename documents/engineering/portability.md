...
| Inference executor placement | the supported three-role daemon model splits Pulsar coordination (`infernix-coordinator`) from engine execution pools. Coordinators route through `CompiledPlacement` / `CompiledDaemon`; engine subscription and launch use live-refined `RuntimePlan` / `ExecutableModel`. No daemon has a PVC; engine pods use ephemeral `emptyDir` for the model-weight cache only. The indexed compile/refine/capped-engine contract is owned by [../architecture/bounded_inference_memory.md](../architecture/bounded_inference_memory.md). See [../architecture/daemon_topology.md](../architecture/daemon_topology.md), [../architecture/engine_pool_routing.md](../architecture/engine_pool_routing.md), and [object_storage.md](object_storage.md). | the cluster-side coordinator hands batches to Apple host-daemon pool members over derived Pulsar topics; normal pools use `Shared` and pinned routes use `Exclusive`. The compiler accounts against checked unified host RAM capacity, oversized rows remain explicit unavailable placements, and live Apple refinement promotes only matching grant/enforcer pairs to executable models under the opaque serialization authority and fixed observer | the cluster-side coordinator routes to separate Kubernetes engine workloads. Linux CPU compilation uses engine-pod capacity and refinement verifies the process-group/cgroup enforcer shape. Linux GPU execution requires independently indexed host-RAM and GPU-VRAM enforcement; a single-resource plan fails closed with `GpuDualResourceBudgetRequired`. All daemon pods are PVC-free |
