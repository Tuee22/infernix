...
- The Apple Metal/Core ML native engine materialization lane is Apple substrate detail, not part
  of the portable contract: it uses typed engine-artifact manifests and public upstream
  MLX/coremltools APIs without repository-owned native source. No Tart or native bridge helper path
  exists in the host-tool schema or prerequisite contract.
...
prerequisites substrate-specific. Apple Metal/Core ML materialization avoids Tart,
...
- Python-native adapter source and quality remain shared under `python/`, while inference executes
  through the canonical per-engine Poetry environment prepared for the active substrate
...
| Bootstrap activation boundary | stage-0 bootstrap surfaces continue in the current process only after they can verify the executable they need next, and they stop for explicit rerun when a new shell or reboot is required | the bootstrap verifies the selected ghcup-managed `ghc` and `cabal` executables before direct `cabal install`; tracked exact Haskell protobuf output removes any Darwin compiler/plugin prerequisite | Linux bootstraps stop for Docker group-membership re-entry and NVIDIA-driver reboot, then continue through the same bootstrap surface on rerun; the Linux image build separately runs the pinned protobuf byte-regeneration gate |
...
| Python adapter environment | keep the shared project framework-free and prepare only the canonical per-engine framework plan; runtime verifies the exact interpreter and marker and never installs on request | `python/.venv/` plus `python/engines/{transformers,pytorch,diffusers}/.venv/`; the binary installs the `apple-silicon` groups under project locks after reconciling Python/Poetry, then publishes post-install digest markers | `linux-cpu` uses the same binary producer for `transformers`/`pytorch` during the base-image materialization step; `linux-gpu` keeps each `cuda` venv in its selected engine image |
| Browser E2E runner | exercise the routed surface for the active generated catalog | host-native `npm exec` runs Playwright against the published localhost edge port using the same typed fixture | the outer container runs `npm --prefix web exec -- playwright test` inside the substrate image against the routed cluster on Docker's private `kind` network |
| Inference executor placement | the supported three-role daemon model splits Pulsar coordination (`infernix-coordinator`) from engine execution pools. Coordinators route through `CompiledPlacement` / `CompiledDaemon`; engine subscription and launch use live-refined `RuntimePlan` / `ExecutableModel`. No daemon has a PVC; engine pods use ephemeral `emptyDir` for the model-weight cache only. Each model's requirement is derived from its own artifact and indexed per physical resource, and each substrate declares the enforcement strength it actually has rather than a strength it merely wants; that contract is owned by [../architecture/bounded_inference_memory.md](../architecture/bounded_inference_memory.md). See [../architecture/daemon_topology.md](../architecture/daemon_topology.md), [../architecture/engine_pool_routing.md](../architecture/engine_pool_routing.md), and [object_storage.md](object_storage.md). | the cluster-side coordinator hands batches to Apple host-daemon pool members over derived Pulsar topics; normal pools use `Shared` and pinned routes use `Exclusive`. The compiler accounts the derived requirement against checked unified host RAM capacity, oversized rows remain explicit unavailable placements, and live Apple refinement promotes only matching grant/enforcer pairs to executable models under the opaque serialization authority and fixed observer. This substrate declares detection: unified memory, no cgroups, and an address-space limit aliased to an advisory limit that rejects every finite ceiling leave nothing to install before the engine's first allocation, so the fixed sampler is the enforcement rather than a second line behind one | the cluster-side coordinator routes to separate Kubernetes engine workloads. Linux CPU compilation uses engine-pod capacity and refinement verifies the process-group/cgroup enforcer shape; once its ceiling is calibrated on the lane, a fixed public-tool launch prefix lowers the data-segment limit and is replaced by the engine image, so private writable growth is refused rather than sampled, with detection retained over the residue. Linux GPU execution requires independently indexed host-RAM and GPU-VRAM enforcement; a single-resource plan fails closed with `GpuDualResourceBudgetRequired`, and the device half is admission plus arena sizing plus detection because no kernel mechanism bounds device memory. All daemon pods are PVC-free |
...
- `infernix test integration` and `infernix test e2e` validate the generated active-mode catalog
  and routed surface against the active initialized substrate instead of silently substituting another
  substrate.
