...
- the routed Playwright contract cross-checks `/api/models` against the serialized generated catalog file reported through `/api/publication` while driving the real routed cluster edge
...
- manual inference requests execute through process-isolated engine-worker adapters backed by durable runtime artifact bundles and direct-upstream source-artifact manifests; automated validation uses the repo-owned default engine probe command while supported-host third-party engine validation remains open
