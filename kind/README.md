...
- `cluster-apple-silicon.yaml` defines the Apple host-compatible Kind shape, including the shared
  edge or registry or MinIO or Pulsar port mappings and the registry-host mount under
  `./.build/kind/apple-silicon/registry`
- `cluster-linux-cpu.yaml` defines the CPU validation topology with the same routed port and
  registry-host contract under `./.build/kind/linux-cpu/registry`
- `cluster-linux-gpu.yaml` defines the `nvkind` template for the supported `linux-gpu` lane,
  including the GPU worker labels, registry-host contract under
  `./.build/kind/linux-gpu/registry`, and CDI device mount used by the NVIDIA-backed worker node
