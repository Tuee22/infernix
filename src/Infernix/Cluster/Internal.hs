...
-- | Trusted lifecycle implementation. Public callers use the closed program
-- facade; the ordinary-IO callbacks here are package-private runtime brackets.
module Infernix.Cluster.Internal
...
import Infernix.Cluster.LifecycleProgram.Internal qualified as Lifecycle
...
import Infernix.Evidence.Lease.Internal (Acquire (..), Lease, leasePayload, withLease)
...
-- | The public runner owns path discovery as well as acquisition. A caller
-- cannot select another lock location for the same configured cluster.
runClusterLifecycle ::
  (forall s. Lifecycle.LifecycleSession s %1 -> Lifecycle.LifecycleProgram s result) ->
  IO result
runClusterLifecycle program = do
  paths <- discoverClusterCommandPaths
  Config.ensureRepoLayout paths
  runClusterLifecycleAt paths program

-- | Internal fixture seam. The exported facade does not expose path selection,
-- the held lease, program constructors, or an arbitrary effect interpreter.
runClusterLifecycleAt ::
  Paths ->
  (forall s. Lifecycle.LifecycleSession s %1 -> Lifecycle.LifecycleProgram s result) ->
  IO result
runClusterLifecycleAt paths program =
  withClusterLifecycleLock paths $ \held ->
    interpretLifecycle paths held (program Lifecycle.LifecycleSession)

interpretLifecycle :: Paths -> Lease s ClusterMutationLocked -> Lifecycle.LifecycleProgram s result -> IO result
interpretLifecycle paths held program =
  case program of
    Lifecycle.FinishLifecycle result -> pure result
    Lifecycle.ObserveLifecycle next -> do
      state <- loadClusterState paths
      interpretLifecycle paths held (next state Lifecycle.LifecycleSession)
    Lifecycle.PerformLifecycle operation next -> do
      case operation of
        Lifecycle.OperatorUp mode -> clusterUpForOwnerUnderLock held paths SOperatorOwned mode
        Lifecycle.OperatorDown mode -> runLifecycleDown held paths "tear down the operator cluster" SOperatorOwned mode
        Lifecycle.HarnessUp mode -> clusterUpForOwnerUnderLock held paths SHarnessOwned mode
        Lifecycle.HarnessDown mode -> runLifecycleDown held paths "tear down the harness cluster" SHarnessOwned mode
        Lifecycle.HarnessGpuEngine engine -> rotateHarnessGpuEngineUnderLock held paths engine
      interpretLifecycle paths held (next Lifecycle.LifecycleSession)
...
withPersistedClusterMutation paths expectedState phaseName detail body =
  withClusterLifecycleLock paths $ \lifecycleLock ->
    withPersistedClusterMutationUnderLock lifecycleLock paths expectedState phaseName detail body

withPersistedClusterMutationUnderLock ::
  Lease s ClusterMutationLocked ->
  Paths ->
  ClusterState ->
  String ->
  String ->
  (ClusterState -> IO result) ->
  IO result
withPersistedClusterMutationUnderLock lifecycleLock paths expectedState phaseName detail body = do
...
-- | Rotate only the binary-owned GPU engine deployment inventory. Every call
-- discovers the configured resource and reacquires authority before mutation.
prepareLinuxGpuEngineDeployment :: Maybe Text.Text -> IO ()
prepareLinuxGpuEngineDeployment engine =
  runClusterLifecycle (\session -> Lifecycle.lifecycleHarnessGpuEngine session engine (Lifecycle.finishLifecycle ()))

rotateHarnessGpuEngineUnderLock :: Lease s ClusterMutationLocked -> Paths -> Maybe Text.Text -> IO ()
rotateHarnessGpuEngineUnderLock held paths requestedEngine = do
  let engines = perEngineDeploymentNames LinuxGpu
  when (maybe False (`notElem` engines) requestedEngine) $
    ioError (userError "harness GPU engine rotation names an engine outside the generated deployment inventory")
  state <- maybe (ioError (userError "harness GPU engine rotation requires persisted cluster state")) pure =<< loadClusterState paths
  unless (clusterOwner state == HarnessOwned && clusterRuntimeMode state == LinuxGpu) $
    ioError (userError "harness GPU engine rotation requires a HarnessOwned linux-gpu cluster")
  withPersistedClusterMutationUnderLock
    held
    paths
    state
    "linux-gpu-engine-deployment-rotation"
    ("rotating the GPU engine deployments to " <> maybe "the shared deployment" Text.unpack requestedEngine)
    (\fresh -> rotateGpuEngineDeployments paths fresh engines requestedEngine)

rotateGpuEngineDeployments :: Paths -> ClusterState -> [Text.Text] -> Maybe Text.Text -> IO ()
rotateGpuEngineDeployments paths state engines requestedEngine = do
  ensureOuterContainerKindNetworkAccess paths LinuxGpu
  ensureClusterKubeconfigPresent paths state
  let scale workload count = runClusterCommand paths (Command.kubectlScaleDeployment (clusterKubeTarget state) (Command.Namespace "platform") (Command.WorkloadRef workload) count)
      deployment engine = "deployment/infernix-engine-" <> Text.unpack engine
  when (isJust requestedEngine) (scale "deployment/infernix-engine" 0)
  forM_ engines $ \engine -> scale (deployment engine) (if Just engine == requestedEngine then 1 else 0)
  when (isNothing requestedEngine) (scale "deployment/infernix-engine" 1)
  let active = maybe "deployment/infernix-engine" deployment requestedEngine
  runClusterCommand paths (Command.kubectlRolloutStatus (clusterKubeTarget state) (Command.Namespace "platform") (Command.WorkloadRef active) 900)
...
clusterUp mode = runClusterLifecycle (\session -> Lifecycle.lifecycleOperatorUp session mode (Lifecycle.finishLifecycle ()))

clusterUpHarness :: Maybe RuntimeMode -> IO ()
clusterUpHarness mode = runClusterLifecycle (\session -> Lifecycle.lifecycleHarnessUp session mode (Lifecycle.finishLifecycle ()))

clusterUpForOwnerUnderLock :: Lease s ClusterMutationLocked -> Paths -> SClusterOwner owner -> Maybe RuntimeMode -> IO ()
clusterUpForOwnerUnderLock lifecycleLock paths ownerSingleton maybeRuntimeMode = do
...
clusterUpWithPulsarBootstrapRepair lifecycleLock teardownAuthority ownerSingleton paths runtimeMode = go 0
  where
    ...
    go repairAttempts = do
      reconcileInterruptedClusterTeardown lifecycleLock teardownAuthority paths runtimeMode
      ...
      reconcileInterruptedClusterMutation lifecycleLock teardownAuthority paths runtimeMode $ do
        repairAttempts' <-
...
reconcileInterruptedClusterTeardown :: Lease s ClusterMutationLocked -> ClusterTeardownAuthority owner s -> Paths -> RuntimeMode -> IO ()
reconcileInterruptedClusterTeardown lifecycleLock teardownAuthority paths runtimeMode = do
  maybeState <- loadClusterState paths
        ...
        ClusterTearingDown phase -> do
          ...
          if clusterExists
            then do
              ...
              requireLifecycleEffectAuthority EffectNeedsPresentCluster lifecycleLock teardownAuthority paths runtimeMode (Just state)
              unpauseSnapshotWorkers paths runtimeMode workerNodes
            else do
              requireLifecycleEffectAuthority EffectNeedsAbsentCluster lifecycleLock teardownAuthority paths runtimeMode (Just state)
...
-- evidence.
reconcileInterruptedClusterMutation :: Lease s ClusterMutationLocked -> ClusterTeardownAuthority owner s -> Paths -> RuntimeMode -> IO a -> IO a
reconcileInterruptedClusterMutation lifecycleLock teardownAuthority paths runtimeMode =
  runInterruptedMutationRecovery
    InterruptedMutationRecoveryEffects
      { observeMutationRecoveryState =
        ...
        uncordonMutationNodes = \state -> do
          requireLifecycleEffectAuthority EffectNeedsPresentCluster lifecycleLock teardownAuthority paths runtimeMode (Just state)
          result <- uncordonAllNodes state
          requireLifecycleEffectAuthority EffectNeedsPresentCluster lifecycleLock teardownAuthority paths runtimeMode (Just state)
          pure result,
...
clusterUpWithPlatform lifecycleLock teardownAuthority ownerSingleton paths runtimeMode = do
  ...
  unless clusterAlreadyPresent $ do
    withWriterQuiesced lifecycleLock paths runtimeMode $ \quiesced -> do
      ...
      requireLifecycleEffectAuthority EffectNeedsAbsentCluster lifecycleLock teardownAuthority paths runtimeMode (Just provisionalState)
  ...
  requireLifecycleEffectAuthority EffectNeedsPresentCluster lifecycleLock teardownAuthority paths runtimeMode (Just warmModelCacheState)
...
revalidateClusterTeardownAuthority lifecycleLock action authority paths runtimeMode = do
  ...
  recordedState <- loadClusterState paths
  presentRuntimeModes <- presentClusterRuntimeModes paths
  requireExactClusterState paths recordedState
  currentReservationAccess <-
  ...
  _ <-
    requireClusterOwnership
      ...
      recordedState
  requireExactClusterState paths recordedState
  finalReservationAccess <- requireReservationAccess paths (teardownAuthorityOwnerValue authority)
  unless (finalReservationAccess == currentReservationAccess) $
    ioError (userError "cluster teardown authority reservation identity changed during ownership observation")
  pure (recordedState, presentRuntimeModes)

requireExactClusterState :: Paths -> Maybe ClusterState -> IO ()
requireExactClusterState paths expected = do
  current <- loadClusterState paths
  unless (current == expected) $
    ioError (userError "cluster state changed after lifecycle authorization")

data LifecycleEffectPresence = EffectNeedsPresentCluster | EffectNeedsAbsentCluster

requireLifecycleEffectAuthority ::
  LifecycleEffectPresence ->
  Lease s ClusterMutationLocked ->
  ClusterTeardownAuthority owner s ->
  Paths ->
  RuntimeMode ->
  Maybe ClusterState ->
  IO ()
requireLifecycleEffectAuthority presence lifecycleLock authority paths runtimeMode expected = do
  (current, present) <-
    revalidateClusterTeardownAuthority lifecycleLock "complete the lifecycle effect" authority paths runtimeMode
  unless (current == expected) $
    ioError (userError "cluster state changed before lifecycle effect")
  let presenceMatches = case presence of
        EffectNeedsPresentCluster -> runtimeMode `elem` present
        EffectNeedsAbsentCluster -> runtimeMode `notElem` present
  unless presenceMatches $
    ioError (userError "cluster writer presence changed before lifecycle effect")
...
-- The private transaction runner captures the exact durable reservation before
-- setup, and installs restoration custody before allowing setup to fail. No
-- callback receives reservation or lifecycle authority.
withHarnessConfigTransaction :: Paths -> Bool -> IO () -> IO a -> IO () -> IO a
withHarnessConfigTransaction paths hadExistingRuntimeConfig setup action restoreConfig =
  mask $ \restore -> do
    reservation <-
      withClusterLifecycleLock paths $ \_ ->
        beginHarnessConfigReservation paths hadExistingRuntimeConfig
    finallyPreservingPrimary
      ( do
          withClusterLifecycleLock paths $ \_ -> do
            requireExactHarnessReservation paths reservation
            restore setup
          restore action
      )
      (completeHarnessConfigTransaction paths reservation restoreConfig)

beginHarnessConfigTransaction :: Paths -> Bool -> IO a -> IO a
beginHarnessConfigTransaction paths hadExistingRuntimeConfig action =
  withClusterLifecycleLock paths $ \_ -> do
    _ <- beginHarnessConfigReservation paths hadExistingRuntimeConfig
    action

beginHarnessConfigReservation :: Paths -> Bool -> IO HarnessReservation
beginHarnessConfigReservation paths hadExistingRuntimeConfig = do
  ...
  case harnessReservationConfigTransaction reservation of
    HarnessConfigUntouched -> do
      ...
      requireExactHarnessReservation paths reservation
      writeHarnessReservation paths pending
      pure pending
    transaction ->
      ioError
        ( userError
...
completeHarnessConfigTransaction :: Paths -> HarnessReservation -> IO () -> IO ()
completeHarnessConfigTransaction paths reservation action =
  withClusterLifecycleLock paths $ \_ -> do
    access <- requireReservationAccess paths HarnessOwned
    current <- harnessReservationFromAccess access
    unless (current == reservation) $
      ioError (userError "harness config transaction reservation identity changed after authorization")
    Subprocess.withBoundedCommandActivitiesQuiescent
      paths
...
requireExactHarnessReservation :: Paths -> HarnessReservation -> IO ()
requireExactHarnessReservation paths expected = do
  current <- readHarnessReservation paths
  unless (current == Just expected) $
    ioError (userError "harness reservation identity changed after authorization")
                            ...
                            ( \activitiesQuiescent -> do
                                ...
                                if deadReservationCanBeRemoved restoredReservation presentRuntimeModes recordedState
                                  then
                                    ...
                                    requireExactHarnessReservation paths lockedReservation
                                    writeHarnessReservation paths restoredReservation
                            )
                      )
...
clusterDown mode = runClusterLifecycle (\session -> Lifecycle.lifecycleOperatorDown session mode (Lifecycle.finishLifecycle ()))
...
clusterDownHarness mode = runClusterLifecycle (\session -> Lifecycle.lifecycleHarnessDown session mode (Lifecycle.finishLifecycle ()))
...
runLifecycleDown :: Lease s ClusterMutationLocked -> Paths -> String -> SClusterOwner owner -> Maybe RuntimeMode -> IO ()
runLifecycleDown lifecycleLock paths action requestedOwnerSingleton maybeRuntimeMode = do
  reservationAccess <- requireReservationAccess paths (clusterOwnerValue requestedOwnerSingleton)
  clusterDownForOwnerUnderLock lifecycleLock paths action requestedOwnerSingleton reservationAccess maybeRuntimeMode
...
-- The private callback must finish retained-state repair before returning from
-- the 'WriterQuiesced' bracket. This is a trusted runtime obligation; its rank-2
-- ordinary-IO signature does not independently prove nonescape.
clusterDownResolved ::
  Lease s ClusterMutationLocked ->
  ClusterTeardownAuthority owner s ->
  Paths ->
        ...
        if usesHostBindMounts
          then deleteRecordedCluster maybeState
            ...
            withFrozenRetainedSnapshotSource
              ...
              frozenMaybeState
              ( \frozenSource -> do
                  snapshotState <- withDetachedRetainedCopyTarget lifecycleLock paths runtimeMode $ \detachedTarget ->
                    case frozenMaybeState of
                      Just state -> do
                        ...
                        syncKindNodeRuntimePathsToHost lifecycleLock authority frozenSource detachedTarget paths (Just replayState)
                      Nothing ->
                        syncKindNodeRuntimePathsToHost lifecycleLock authority frozenSource detachedTarget paths Nothing
                  -- The frozen-source lease remains live through deletion. Its
                  -- release then tolerates the now-absent worker containers.
                  deleteRecordedCluster snapshotState
              )
      else pure recordedState
  withWriterQuiesced lifecycleLock paths runtimeMode $ \quiesced -> do
    requireLifecycleEffectAuthority EffectNeedsAbsentCluster lifecycleLock authority paths runtimeMode terminalState
    scrubRetainedStateUnderLease quiesced paths
    onQuiesced quiesced
  requireLifecycleEffectAuthority EffectNeedsAbsentCluster lifecycleLock authority paths runtimeMode terminalState
  case matchingClusterState runtimeMode terminalState of
...
-- | A private credential bracket for the closed realm reconciliation operation.
-- Each attempt requests a fresh bearer. The rank-2 IO callback does not prove
-- nonescape or remote validity: payload access stays in this module and the
-- server may reject an expired/revoked token, which fails the attempt.
withValidAdminToken ::
  Manager ->
  String ->
...
cleanupHarnessRuntimeState paths runtimeMode =
  withClusterLifecycleLock paths $ \lifecycleLock -> do
    reservation <- requireReservationAccess paths HarnessOwned
    maybeState <- loadClusterState paths
    case maybeState of
      Just state
        | clusterOwner state == OperatorOwned,
      ...
      _ -> pure ()
    withAllClusterWritersQuiesced lifecycleLock paths $ \quiesced -> do
      currentReservation <- requireReservationAccess paths HarnessOwned
      unless (currentReservation == reservation) $
        ioError (userError "harness runtime cleanup reservation identity changed after authorization")
      requireExactClusterState paths maybeState
...
removeHarnessRuntimeStateUnderLease quiesced paths =
  case leasePayload quiesced of
    AllClusterWritersQuiesced -> do
      ...
      forM_ entries $ \entry ->
        unless
          (entry `elem` ["bounded-command-activity", "locks", "secrets", "validation"])
...
syncKindNodeRuntimePathsToHost ::
  Lease lock ClusterMutationLocked ->
  ClusterTeardownAuthority owner lock ->
  Lease source FrozenRetainedSnapshotSource ->
  Lease target DetachedRetainedCopyTarget ->
  Paths ->
  Maybe ClusterState ->
  IO (Maybe ClusterState)
syncKindNodeRuntimePathsToHost lifecycleLock authority frozenSource detachedTarget paths maybeState =
  case (leasePayload frozenSource, leasePayload detachedTarget) of
    (FrozenRetainedSnapshotSource sourceRuntimeMode workerNodes claimNodeBindings, DetachedRetainedCopyTarget runtimeMode) -> do
      ...
      let localKindRoot = kindRuntimeRoot paths runtimeMode
          ...
          checkCustody expected = do
            requireLifecycleEffectAuthority EffectNeedsPresentCluster lifecycleLock authority paths runtimeMode expected
            forM_ workerNodes $ \nodeName -> do
              observed <- tryClusterCommand paths (Command.dockerContainerPaused (Command.ContainerName nodeName))
              unless (classifyWorkerPauseObservation observed == Right WorkerAlreadyPaused) $
                ioError (userError "retained snapshot writer is no longer proven paused")
            requireExactClusterState paths expected
            currentReservation <- requireReservationAccess paths (teardownAuthorityOwnerValue authority)
            unless (currentReservation == teardownAuthorityReservationAccess authority) $
              ioError (userError "snapshot reservation identity changed during writer observation")
          stageCopy expected nodeName containerDirectory stagedDirectory detail = do
            checkCustody expected
            next <- traverse (\state -> startLifecyclePhase paths state "cluster-down" "replay-retained-state" detail) expected
            ...
            checkCustody next
            pure next
          stageClaim expected persistentClaim =
            case Map.lookup (persistentVolumeClaimName persistentClaim) claimNodeBindings of
              Nothing -> ioError (userError ("retained snapshot staging found no owning node for claim " <> persistentVolumeClaimName persistentClaim))
              Just nodeName ->
                stageCopy
                  expected
                  nodeName
                  (nodeMountedClaimPath persistentClaim)
                  (claimDirectoryUnder stagingRoot persistentClaim)
                  ("staging claim " <> persistentVolumeClaimName persistentClaim <> " from " <> nodeName)
          stageWorker expected nodeName =
            stageCopy
              expected
              nodeName
              nodeMountedKindRoot
          ...
          stageSnapshot =
            case maybeState of
              Just state
                | not (null (claims state)) ->
                    foldM stageClaim maybeState (filter (not . isPatroniManagedClaim) (claims state))
              _ -> foldM stageWorker maybeState workerNodes
      checkCustody maybeState
      reconcileInterruptedRetainedSnapshot paths runtimeMode
      createDirectoryIfMissing True stagingRoot
      stagedState <- stageSnapshot
      checkCustody stagedState
      writeFile (snapshotCompletionMarkerPath stagingRoot) "version=1\n"
