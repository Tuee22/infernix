...
--   disabled guard.
-- * @linux-cpu@: admission uses the 4 GiB child-execution budget. The pod
--   limit is a larger 5 GiB daemon + child + watchdog envelope.
...
-- | Observe the current Apple host partition without applying a configured
-- fallback. Runtime-plan refinement uses this live fact to reject a configured
-- partition that no longer matches physical RAM or the running Colima pledge.
observeAppleHostMemoryPartition :: Paths -> IO (Maybe HostMemoryPartition)
observeAppleHostMemoryPartition paths = do
  observed <-
    try
      ( do
          hostConfig <- HostConfig.decodeHostConfigFile (Config.hostConfigPath paths)
  ...
  pure $
    case observed :: Either SomeException (Int, Int) of
      Left _ -> Nothing
      Right (physicalMib, colimaMib) ->
        either
          (const Nothing)
...
requireFallbackPartition capacityMib =
  case hostPartitionForCapacity capacityMib of
    ...
    Left partitionError ->
      ioError
        (userError ("invalid built-in Apple host memory fallback: " <> partitionError))
...
-- | The total memory pledged to Colima profiles that are not explicitly
